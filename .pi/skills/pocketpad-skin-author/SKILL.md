...
1. **Art direction** — run the `pocketpad-art-director` agent. Require `reviews/art-direction.md` before source changes.
2. **Execution** — run the `pocketpad-skin-designer` agent to author JSON/SVG, compile, validate, quality-check, and render `reviews/contact-sheet-1.png`.
3. **Independent critique** — run `pocketpad-visual-critic` against the exact sheet. The parent must read and synthesize findings; then give the designer exact corrective changes. Never delegate “fix whatever the critic found.”
...
5. **QA** — run `pocketpad-skin-qa`. A passing package must compile byte-for-byte identically twice and pass both strict validators.
