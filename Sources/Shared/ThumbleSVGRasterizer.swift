...
public enum ThumbleSVGRasterizerError: Error, LocalizedError, Equatable {
    case sourceTooLarge
...
public enum ThumbleSVGRasterizer {
    ...
    public static func sanitize(_ data: Data) throws -> Data {
        guard data.count <= maximumSourceBytes else { throw ThumbleSVGRasterizerError.sourceTooLarge }
        guard var source = String(data: data, encoding: .utf8) else { throw ThumbleSVGRasterizerError.cannotDecode }
        source = source.trimmingCharacters(in: .whitespacesAndNewlines)
        let lower = source.lowercased()
        guard lower.hasPrefix("<svg") || lower.hasPrefix("<?xml") else {
            throw ThumbleSVGRasterizerError.cannotDecode
        }
        ...
        if let expression = try? NSRegularExpression(pattern: urlPattern, options: [.caseInsensitive]) {
            ...
            for match in expression.matches(in: source, range: range) {
                ...
                if !value.hasPrefix("#") {
                    throw ThumbleSVGRasterizerError.unsafeConstruct("external CSS URL")
                }
            }
        }
        ...
        else { throw ThumbleSVGRasterizerError.excessiveComplexity }
    ...
    public static func rasterize(_ data: Data, width: Int, height: Int) throws -> Data {
        guard (16...4096).contains(width), (16...4096).contains(height) else {
            throw ThumbleSVGRasterizerError.invalidDimensions
        }
        let sanitized = try sanitize(data)
        #if os(macOS)
        guard let image = NSImage(data: sanitized) else { throw ThumbleSVGRasterizerError.cannotDecode }
        return try drawPNG(width: width, height: height) { rect in
            image.draw(in: rect, from: .zero, operation: .sourceOver, fraction: 1)
        }
        #else
        throw ThumbleSVGRasterizerError.unsupportedPlatform
        #endif
    }
    ...
    private static func drawPNG(
        ...
        guard let bitmap = NSBitmapImageRep(
            ...
            bytesPerRow: 0,
            bitsPerPixel: 0
        ) else { throw ThumbleSVGRasterizerError.cannotRender }
        bitmap.size = NSSize(width: width, height: height)
        NSGraphicsContext.saveGraphicsState()
        defer { NSGraphicsContext.restoreGraphicsState() }
        guard let graphics = NSGraphicsContext(bitmapImageRep: bitmap) else {
            throw ThumbleSVGRasterizerError.cannotRender
        }
