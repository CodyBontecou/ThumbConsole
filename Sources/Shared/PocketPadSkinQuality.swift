...
public enum PocketPadSkinQualitySeverity: String, Codable, Equatable, Sendable {
    case error
    case warning
}

public struct PocketPadSkinQualityIssue: Codable, Equatable, Identifiable, Sendable {
    public var severity: PocketPadSkinQualitySeverity
    public var code: String
    public var message: String
    public var path: String?

    public init(
        severity: PocketPadSkinQualitySeverity,
        code: String,
        message: String,
        path: String? = nil
    ) {
...
public struct PocketPadSkinQualityReport: Codable, Equatable, Sendable {
    public var issues: [PocketPadSkinQualityIssue]
    public var score: Int
    public var checkedArtboardID: String?

    public init(issues: [PocketPadSkinQualityIssue], checkedArtboardID: String? = nil) {
        self.issues = issues
    ...
    public var errors: [PocketPadSkinQualityIssue] { issues.filter { $0.severity == .error } }
    public var warnings: [PocketPadSkinQualityIssue] { issues.filter { $0.severity == .warning } }
...
public enum PocketPadSkinQualityEvaluator {
    public static func evaluate(
        package: PocketPadSkinPackage,
        workspace: PocketPadSkinWorkspace? = nil,
        artboardID requestedArtboardID: String? = nil
    ) -> PocketPadSkinQualityReport {
        var issues: [PocketPadSkinQualityIssue] = []
        func add(_ severity: PocketPadSkinQualitySeverity, _ code: String, _ message: String, path: String? = nil) {
        ...
        let packageReport = PocketPadSkinPackageValidator.validate(package)
        ...
        if let workspace {
            let sourceReport = PocketPadSkinSourceValidator.validate(workspace)
        ...
        let artboard = artboardID.flatMap(PocketPadSkinArtboardCatalog.resolve)
        if let artboard {
        ...
        return PocketPadSkinQualityReport(issues: ordered, checkedArtboardID: artboard?.id)
    }

    private static func evaluateAuthorship(
        _ workspace: PocketPadSkinWorkspace,
        add: (PocketPadSkinQualitySeverity, String, String, String?) -> Void
    ) {
    ...
    private static func evaluateArtboard(
        _ artboard: PocketPadSkinArtboard,
        package: PocketPadSkinPackage,
        ...
        guard let profile = PocketPadSkinArtboardCatalog.profile(for: artboard.id) else {
        ...
        for variant in artboard.variants {
            ...
            let safeRect = PocketPadNormalizedRect(
                x: safe.leading,
                y: safe.top,
    ...
    private static func evaluateVariantMatrix(
        ...
        artboard: PocketPadSkinArtboard?,
        add: (PocketPadSkinQualitySeverity, String, String, String?) -> Void
    ) {
        ...
        let orientations = package.manifest.compatibility?.normalized.orientations.isEmpty == false
            ...
            : (artboard?.variants.map(\.orientation) ?? PocketPadSkinOrientation.allCases)
        for orientation in Set(orientations) {
            for scheme in PocketPadSkinColorScheme.allCases {
    ...
    private static func evaluateAssetDimensionsAndBudgets(
        ...
        if let archive = try? PocketPadSkinPackageCodec.encode(package), archive.count > 30 * 1024 * 1024 {
    ...
    private static func evaluateLayerSafety(
        ...
        for layer in layers {
            ...
            if layer.plane == .overlay && layer.opacity > 0.92 && layer.frame?.normalized == PocketPadNormalizedRect(x: 0, y: 0, width: 1, height: 1) {
    ...
    private static func inferredOrientation(from id: String) -> PocketPadSkinOrientation? {
