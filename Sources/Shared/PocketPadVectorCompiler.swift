...
public enum PocketPadVectorCompilerError: Error, LocalizedError, Equatable {
...
public struct PocketPadCompiledCanvas: Equatable, Sendable {
...
public enum PocketPadVectorCompiler {
    public static func compileCanvases(
        ...
        for orientation in orientations {
            ...
            for scheme in schemes {
                ...
                var png = try PocketPadSVGRasterizer.rasterize(Data(svg.utf8), width: width, height: height)
                for source in workspace.sourceAssets where source.purpose == .canvasArtwork {
                    ...
                    guard sourceURL.pathExtension.lowercased() == "svg" else {
                        throw PocketPadVectorCompilerError.unsupportedSourceFormat(source.path)
                    }
                    let overlay = try PocketPadSVGRasterizer.rasterize(
                        Data(contentsOf: sourceURL, options: [.mappedIfSafe]),
                        width: width,
                        height: height
                    )
                    png = try PocketPadSVGRasterizer.composite(
                        basePNG: png,
                        overlayPNG: overlay,
                        width: width,
                        height: height
                ...
                guard png.count <= GamepadImageFill.maximumStoredBytes else {
                    throw PocketPadVectorCompilerError.outputTooLarge(id)
                }
                canvases.append(PocketPadCompiledCanvas(
                    id: id,
    ...
    public static func generatedSVG(
        workspace: PocketPadSkinWorkspace,
        artboard: PocketPadSkinArtboardVariant,
        ...
        for (index, component) in workspace.components.enumerated() {
            ...
            guard let material = materials[component.materialID] else {
                throw PocketPadVectorCompilerError.missingMaterial(component.materialID)
            }
    ...
    private static func safeSourceURL(
        ...
        guard PocketPadSkinPackageCodec.isSafePackagePath(path), path.hasPrefix("sources/") else {
            throw PocketPadVectorCompilerError.unsafeSourcePath(path)
        }
        ...
        guard fileManager.fileExists(atPath: candidate.path) else {
            throw PocketPadVectorCompilerError.missingSource(path)
        }
    ...
    private static func color(_ value: String) throws -> String {
        guard GamepadRGBAColor(hexString: value) != nil else {
            throw PocketPadVectorCompilerError.invalidColor(value)
        }
