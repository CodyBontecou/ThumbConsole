...
private enum GamepadControlNudgeSolver {
    static func adjustedTranslation(
        ...
        clampedTranslation(translation, snapshots: snapshots, canvasSize: canvasSize)
    }
...
private enum GamepadEditorInteractionMode: String, CaseIterable, Identifiable {
    case edit
    case test

    var id: String { rawValue }
    var displayName: String { rawValue.capitalized }
    var systemImage: String { self == .edit ? "cursorarrow" : "play.fill" }
}

private enum GamepadConfigurationSidebarTab: String, CaseIterable, Identifiable {
    case setups
    case layers
...
private enum GamepadEditorInspectorMode: String, CaseIterable, Identifiable {
    case basic
    case advanced
...
private struct GamepadPendingOrientationChange: Identifiable {
    let id = UUID()
...
private struct GamepadMarqueeSelectionState {
    let startPoint: CGPoint
    var currentPoint: CGPoint
    let baseSelection: Set<GamepadControlIdentity>
...
private enum GamepadResourceLibraryTab: String, CaseIterable, Identifiable {
    case styles
    case assets
...
private struct GeistSegmentedPicker<Option: Hashable>: View {
    ...
    var body: some View {
        ...
        .padding(2)
        .frame(minHeight: Geist.Spacing.s8)
...
struct GamepadCustomizationEditor: View {
    ...
    @ScaledMetric(relativeTo: .body) private var editorCommandBarHeight: CGFloat = 48
    ...
    private let onExportProfiles: (([GamepadConfigurationProfile], UUID, UUID, UUID?) -> Void)?
    ...
    private static let canvasFitInset: CGFloat = 24
    ...
    @State private var isOrientationComparisonPresented = false
    @State private var pendingOrientationChange: GamepadPendingOrientationChange?
    @State private var locallyPressedTestInputs: Set<KeypadElementInputID> = []
    @State private var lastTestActionText: String?
    @State private var editorOperationError: String?
    @State private var setupSearchText = ""
    @State private var layerSearchText = ""
    @State private var hoveredLayerListItemID: String?
    @State private var draggingLayerControlIDs: [GamepadControlIdentity] = []
    @State private var groupRenameTargetID: UUID?
    @State private var groupRenameDraft = ""
    @State private var highlightedQualityControlIDs: Set<GamepadControlIdentity> = []
    @State private var highlightedQualitySeverity: GamepadLayoutIssueSeverity?
    @State private var activeLayoutQualityIssue: GamepadLayoutIssue?
    ...
    @State private var isProfileImporterPresented = false
    @State private var importsProfilesAsCopies = false
    @State private var profileImportNotice: String?
    @State private var isResourceLibraryPresented = false
    @State private var resourceLibraryTab: GamepadResourceLibraryTab = .styles
    @State private var selectedStyleTokenID: String?
    @State private var selectedAssetID: String?
    @State private var isStyleLibraryImporterPresented = false
    @State private var isStyleLibraryExporterPresented = false
    @State private var styleLibraryExportDocument = GamepadStyleLibraryJSONDocument()
    @State private var isAssetImporterPresented = false
    @State private var resourceLibraryNotice: String?
    ...
    @State private var isCanvasFitMode = true
    @State private var canvasViewportSize = CGSize.zero
    @State private var canvasScrollRequestID: UUID?
    @State private var isCanvasFocusMode = false
    @State private var focusModeRestoresConfigurationSidebar = true
    @State private var focusModeRestoresInspectorSidebar = true
    ...
    @State private var canvasZoomValue: Double = 1.0
    ...
    private let onTestElementInputTap: ((KeypadElementInputID, Int) -> Void)?
    ...
    init(
        ...
        onExportProfiles: (([GamepadConfigurationProfile], UUID, UUID, UUID?) -> Void)? = nil,
        ...
        externallyPressedElementInputs: Set<KeypadElementInputID> = [],
        editorDeliveryStatusText: String? = nil,
        onTestElementInputChanged: ((KeypadElementInputID, Bool) -> Void)? = nil,
        ...
        self.onExportProfiles = onExportProfiles
        self.onImportProfiles = onImportProfiles
        ...
        self.externallyPressedElementInputs = externallyPressedElementInputs
        ...
        self._expandedInspectorSections = State(initialValue: [.selectedElementIdentity, .selectedElementPosition, .keypadDevice])
    }
    ...
    private var profileImportNoticePresented: Binding<Bool> {
        Binding(
            get: { profileImportNotice != nil },
            set: { isPresented in
                if !isPresented { profileImportNotice = nil }
            }
        )
    }

    private var resourceLibraryNoticePresented: Binding<Bool> {
        Binding(
            get: { resourceLibraryNotice != nil },
            set: { isPresented in
                if !isPresented { resourceLibraryNotice = nil }
            }
        )
    }
    ...
    private var editorCore: some View {
        ...
        .onChange(of: selectedControlID) { _, _ in
            if selectedControlIsEditable {
                expandedInspectorSections.insert(.selectedElementIdentity)
                expandedInspectorSections.insert(.selectedElementPosition)
            }
        }
        .onChange(of: interactionMode) { oldMode, newMode in
            if oldMode == .test && newMode != .test {
            ...
            if newMode == .test {
                ...
                announceEditor("Test mode. Keypad controls send their configured outputs.")
            } else {
                announceEditor("Edit mode. Keypad controls can be selected and arranged.")
            }
        }
        .onChange(of: editorDeliveryStatusText) { _, status in
            guard let status, status.localizedCaseInsensitiveContains("fail") || status.localizedCaseInsensitiveContains("offline") else { return }
            announceEditor(status)
        }
    ...
    private var presentedEditor: some View {
        editorCore
        .sheet(isPresented: $isResourceLibraryPresented) {
            resourceLibrarySheet
                .frame(minWidth: 720, idealWidth: 820, minHeight: 500, idealHeight: 580)
        }
        .fileImporter(
            isPresented: $isStyleLibraryImporterPresented,
            allowedContentTypes: [.json],
            allowsMultipleSelection: false,
            onCompletion: handleStyleLibraryImport
        )
        .fileExporter(
        ...
        .fileImporter(
            isPresented: $isAssetImporterPresented,
            allowedContentTypes: [.image, .data],
            allowsMultipleSelection: false,
            onCompletion: handleAssetImport
        )
        .fileImporter(
            isPresented: $isProfileImporterPresented,
            allowedContentTypes: [.json],
            allowsMultipleSelection: false,
            onCompletion: handleProfileImport
        )
        .alert(
            "Import Keypad Setups",
            isPresented: profileImportNoticePresented
        ) {
            Button("OK", role: .cancel) { profileImportNotice = nil }
        } message: {
            Text(profileImportNotice ?? "Import complete.")
        }
        .alert(
            "Keypad Resources",
            isPresented: resourceLibraryNoticePresented
        ) {
            Button("OK", role: .cancel) { resourceLibraryNotice = nil }
        } message: {
            Text(resourceLibraryNotice ?? "Resource update complete.")
        }
        .sheet(isPresented: $isShortcutReferencePresented) {
            shortcutReferenceSheet
                .frame(minWidth: 560, idealWidth: 640, minHeight: 470, idealHeight: 560)
        }
        .sheet(isPresented: $isOrientationComparisonPresented) {
            orientationComparisonSheet
                .frame(minWidth: 860, idealWidth: 1040, minHeight: 540, idealHeight: 680)
        }
        .confirmationDialog(
            "Create the missing \(pendingOrientationChange?.frame.orientation.displayName.lowercased() ?? "orientation") layout",
            isPresented: Binding(
                get: { pendingOrientationChange != nil },
                set: { if !$0 { pendingOrientationChange = nil } }
            ),
            titleVisibility: .visible
        ) {
            Button("Copy Current Layout") { completePendingOrientationChange(automaticallyArranged: false) }
            Button("Arrange Automatically") { completePendingOrientationChange(automaticallyArranged: true) }
            Button("Cancel", role: .cancel) { pendingOrientationChange = nil }
        } message: {
            Text("Portrait and landscape are saved separately. You can copy the current positions exactly or create a rotated first pass and review its layout suggestions.")
        }
        .alert(
            "Couldn’t Complete Editor Action",
            isPresented: Binding(
                get: { editorOperationError != nil },
                set: { if !$0 { editorOperationError = nil } }
            )
        ) {
            Button("OK", role: .cancel) { editorOperationError = nil }
        } message: {
            Text(editorOperationError ?? "The requested editor action could not be completed.")
        }
        .alert(
            "Rename Layer Group",
            isPresented: Binding(
                get: { groupRenameTargetID != nil },
                set: { if !$0 { groupRenameTargetID = nil } }
            )
        ) {
            TextField("Group name", text: $groupRenameDraft)
            Button("Rename") { commitGroupRename() }
            Button("Cancel", role: .cancel) { groupRenameTargetID = nil }
        } message: {
            Text("Choose a name that describes the controls in this group.")
        }
        .onDisappear {
            releaseAllTestInputs()
    ...
    private var editorCommandBar: some View {
        GeometryReader { proxy in
            Group {
                if proxy.size.width >= 1_160 {
                    editorCommandBarFullContent
                } else {
                    editorCommandBarCompactContent
                }
            }
            .frame(width: proxy.size.width, height: proxy.size.height)
        }
        .frame(height: min(max(editorCommandBarHeight, 48), 80))
        .background(Geist.color(.background100, scheme: colorScheme))
    }

    private var editorCommandBarFullContent: some View {
        HStack(spacing: Geist.Spacing.s2) {
                if shouldShowCommandBarProfileSwitcher {
                    commandBarProfileSwitcher
                }

                Picker("Mode", selection: $interactionMode) {
                    ForEach(GamepadEditorInteractionMode.allCases) { mode in
                ...
                .labelsHidden()
                .frame(width: 132)
                .help(interactionMode == .edit ? "Edit controls on the canvas" : "Press controls to send their real outputs")
            ...
            commandBarIconButton(
                ...
                label: "Undo",
                isDisabled: !(undoManager?.canUndo ?? false),
                action: { _ = performUndo() }
            )
            commandBarIconButton(
                systemImage: "arrow.uturn.forward",
                label: "Redo",
                isDisabled: !(undoManager?.canRedo ?? false),
                action: { _ = performRedo() }
            )
            ...
            if isControlSelectionActive && interactionMode == .edit {
                commandBarIconButton(
                    ...
                    action: { _ = duplicateSelectedControls() }
                )
                commandBarIconButton(
                    systemImage: "trash",
                    label: "Delete",
                    isDisabled: false,
                    action: { _ = deleteSelectedControl() }
                )
            }

            orientationWorkflowMenu

            layoutQualityMenu

            Spacer(minLength: Geist.Spacing.s2)

            if interactionMode == .test, let lastTestActionText {
                Label(lastTestActionText, systemImage: "waveform")
                    .geistTypography(.label12)
                    .foregroundStyle(Geist.color(.gray900, scheme: colorScheme))
                    .lineLimit(1)
                    .frame(maxWidth: 180)
            ...
            Label(editorSaveAndSyncStatusText, systemImage: editorSaveAndSyncStatusImage)
                .geistTypography(.label12)
                .foregroundStyle(Geist.color(.gray900, scheme: colorScheme))
                .lineLimit(1)
                .help(editorSaveAndSyncStatusText)

            HStack(spacing: 2) {
                commandBarIconButton(systemImage: "minus", label: "Zoom Out", isDisabled: effectiveCanvasZoom <= Self.canvasZoomMin + 0.001) {
                    changeCanvasZoom(by: -0.10)
                }
                Button(canvasZoomPercentageText, action: fitCanvas)
                    .buttonStyle(.plain)
                    .geistTypography(.label12Mono)
                    .frame(minWidth: 42)
                    .accessibilityLabel(isCanvasFitMode ? "Canvas zoom, fit" : "Canvas zoom, \(canvasZoomPercentageText) of fit")
                    .help(isCanvasFitMode ? "Canvas is fit to the workspace (⌘0)" : "\(canvasZoomPercentageText) of fitted size · Click to fit (⌘0)")
                commandBarIconButton(systemImage: "plus", label: "Zoom In", isDisabled: effectiveCanvasZoom >= Self.canvasZoomMax - 0.001) {
                    changeCanvasZoom(by: 0.10)
                }
                if isControlSelectionActive {
                    commandBarIconButton(systemImage: "scope", label: "Zoom to Selection", isDisabled: selectedControlIDs.isEmpty) {
                        zoomToSelection()
                    }
                }
            }
            .padding(.horizontal, Geist.Spacing.s1)
            .background(Geist.color(.gray100, scheme: colorScheme), in: RoundedRectangle(cornerRadius: Geist.Radius.sm, style: .continuous))
            ...
            commandBarIconButton(
                ...
                label: canvasFocusCommandTitle,
                isDisabled: false,
                action: toggleCanvasFocusMode
            )
            ...
            .menuStyle(.button)
                ...
                .accessibilityLabel("Editor help")
            }
        .padding(.horizontal, Geist.Spacing.s3)
        .padding(.vertical, Geist.Spacing.s2)
    }

    private var editorCommandBarCompactContent: some View {
        HStack(spacing: Geist.Spacing.s2) {
            if shouldShowCommandBarProfileSwitcher {
                commandBarProfileSwitcher
                    .frame(maxWidth: 170)
            ...
            Label(editorSaveAndSyncStatusText, systemImage: editorSaveAndSyncStatusImage)
                .labelStyle(.iconOnly)
                .foregroundStyle(Geist.color(.gray900, scheme: colorScheme))
                .accessibilityLabel(editorSaveAndSyncStatusText)
                .help(editorSaveAndSyncStatusText)
    ...
    private var shouldShowCommandBarProfileSwitcher: Bool {
        !isConfigurationSidebarVisible || configurationSidebarTab != .setups
    }

    private var commandBarProfileSwitcher: some View {
        Menu {
            ForEach(profiles) { profile in
                Button {
                    selectProfile(profile)
                } label: {
                    Label(profile.name, systemImage: profile.id == selectedProfileID ? "checkmark" : "rectangle")
                }
            }
            Divider()
            Button("New Blank Setup", action: createProfile)
            Menu("New from Template") {
                ForEach(GamepadControllerTemplate.allCases) { template in
                    Button(template.displayName) { createProfile(from: template) }
                }
            }
        } label: {
            HStack(spacing: Geist.Spacing.s2) {
        ...
        .menuStyle(.button)
        .geistButtonStyle(.secondary, size: .small)
        .help("Switch keypad setup")
    }

    private func commandBarIconButton(
        systemImage: String,
        label: String,
        isDisabled: Bool,
    ...
    private var addControlMenu: some View {
        Button {
            isAddControlPalettePresented.toggle()
        } label: {
    ...
    private var addControlPalette: some View {
        VStack(alignment: .leading, spacing: Geist.Spacing.s4) {
            VStack(alignment: .leading, spacing: Geist.Spacing.s1) {
                ...
                Text("Choose a control, then place or draw it on the iPhone canvas.")
            ...
            Text("Shapes & Decoration")
                .geistTypography(.heading14)
            HStack(spacing: Geist.Spacing.s2) {
                ForEach([GamepadCanvasTool.rectangle, .ellipse, .polygon, .star]) { tool in
                    Button {
    ...
    private func addControlPaletteButton(
        title: String,
        subtitle: String,
        systemImage: String,
        ...
        Button {
            ...
            .overlay(RoundedRectangle(cornerRadius: Geist.Radius.md, style: .continuous).stroke(Geist.color(.grayAlpha400, scheme: colorScheme), lineWidth: 1))
        }
        .buttonStyle(.plain)
        .disabled(isDisabled)
    }

    private var orientationWorkflowMenu: some View {
        Menu {
            ForEach(GamepadEditorDeviceOrientation.allCases) { orientation in
                Button {
                    setDeviceFrame(GamepadEditorDeviceFrame(spec: activeDeviceFrame.spec, orientation: orientation))
                    ...
                    Label(
                        "Edit \(orientation.displayName)",
                        systemImage: orientation == selectedProfileOrientation ? "checkmark" : (hasSavedOrientation(orientation) ? "circle.fill" : "circle")
                    )
                }
            }
            Divider()
            Button("Copy Landscape to Portrait") { transferOrientation(from: .landscape, to: .portrait, automaticallyArranged: false) }
                .disabled(!hasSavedOrientation(.landscape))
            Button("Copy Portrait to Landscape") { transferOrientation(from: .portrait, to: .landscape, automaticallyArranged: false) }
                .disabled(!hasSavedOrientation(.portrait))
            Menu("Arrange Automatically") {
                Button("Landscape to Portrait") { transferOrientation(from: .landscape, to: .portrait, automaticallyArranged: true) }
                    .disabled(!hasSavedOrientation(.landscape))
                Button("Portrait to Landscape") { transferOrientation(from: .portrait, to: .landscape, automaticallyArranged: true) }
                    .disabled(!hasSavedOrientation(.portrait))
            }
            Divider()
            Button("Compare Orientations…") { isOrientationComparisonPresented = true }
            ...
            HStack(spacing: Geist.Spacing.s1) {
                Image(systemName: activeDeviceFrame.orientation == .landscape ? "rectangle" : "rectangle.portrait")
                Text(activeDeviceFrame.orientation.displayName)
                orientationStatusDot(.landscape)
                orientationStatusDot(.portrait)
            }
        }
        .menuStyle(.button)
        .geistButtonStyle(.secondary, size: .small)
        .help("Edit, copy, arrange, or compare portrait and landscape layouts")
    }

    private func orientationStatusDot(_ orientation: GamepadEditorDeviceOrientation) -> some View {
        let isSaved = hasSavedOrientation(orientation)
        return ZStack {
            if isSaved {
                Circle()
                ...
                Circle()
                    .stroke(Geist.color(.gray900, scheme: colorScheme), lineWidth: accessibilityContrast == .increased ? 1.5 : 1)
            }
        }
        .frame(width: accessibilityDifferentiateWithoutColor ? 10 : 7, height: accessibilityDifferentiateWithoutColor ? 10 : 7)
        .accessibilityElement(children: .ignore)
        .accessibilityLabel("\(orientation.displayName) \(isSaved ? "customized" : "not customized")")
    }

    private func hasSavedOrientation(_ orientation: GamepadEditorDeviceOrientation) -> Bool {
        orientation == selectedProfileOrientation || selectedProfile?.hasCustomizationVariant(for: orientation) == true
    }

    private var layoutQualityMenu: some View {
        let report = currentLayoutQualityReport
        let errors = report.issues.filter { $0.severity == .error }
        let warnings = report.issues.filter { $0.severity == .warning }

        return Menu {
            Section {
                Text("\(report.summary.errorCount) errors · \(report.summary.warningCount) warnings")
                Text("Uses \(Int((report.summary.layoutWidthCoverage * 100).rounded()))% width · \(Int((report.summary.layoutHeightCoverage * 100).rounded()))% height")
            }

            if report.issues.isEmpty {
                Label("No layout issues found", systemImage: "checkmark.circle.fill")
            } else {
                if !errors.isEmpty {
                    Section("Errors") {
                        ForEach(errors) { issue in
                            layoutQualityIssueMenuItem(issue)
                        }
                    }
                }
                ...
                if !highlightedQualityControlIDs.isEmpty || highlightedQualitySeverity != nil {
                    Divider()
                    Button("Clear Layout Highlight") {
                        clearLayoutQualityHighlight()
                    }
                }
            }
        } label: {
        ...
        .menuStyle(.button)
        .geistButtonStyle(report.hasErrors ? .error : .secondary, size: .small)
        .help("Review touch-target size, overlap, edge, and canvas-usage suggestions")
    }
    ...
    @ViewBuilder
    private func layoutQualityIssueMenuItem(_ issue: GamepadLayoutIssue) -> some View {
        Menu {
            Button {
                selectLayoutQualityIssue(issue)
            } label: {
                Label("Show on Canvas", systemImage: "scope")
            }

            if !issue.suggestedRepairs.isEmpty {
                Divider()
                ForEach(issue.suggestedRepairs) { repair in
                    Button {
                        selectLayoutQualityIssue(issue)
                        applyLayoutRepair(repair, to: issue)
                    } label: {
                        Label(repair.displayName, systemImage: repair.systemImage)
                    }
                }
            }
        } label: {
            Label(
                layoutIssueMenuTitle(issue),
    ...
    private func layoutIssueMenuTitle(_ issue: GamepadLayoutIssue) -> String {
        issue.controls.isEmpty ? issue.message : issue.message
    }

    private func layoutQualityBadgeText(_ report: GamepadLayoutQualityReport) -> String {
        let prioritizedIssues = report.hasErrors
            ? report.issues.filter { $0.severity == .error }
            : report.issues.filter { $0.severity == .warning }
        guard let firstIssue = prioritizedIssues.first else { return "Layout Good" }

        let matchingCount = prioritizedIssues.filter { $0.code == firstIssue.code }.count
        let remainingCount = prioritizedIssues.count - matchingCount
        let primaryLabel = layoutIssueCategoryLabel(code: firstIssue.code, count: matchingCount)
        return remainingCount > 0 ? "\(primaryLabel) +\(remainingCount)" : primaryLabel
    }

    private func layoutIssueCategoryLabel(code: String, count: Int) -> String {
        let plural = count != 1
        switch code {
        case "no-visible-controls":
            return "No Controls"
        case "control-overlap":
            return plural ? "\(count) Overlaps" : "Overlap"
        case "expanded-hit-overlap", "hit-region-z-order-ambiguous", "hit-region-z-order-mismatch":
            return plural ? "\(count) Touch Conflicts" : "Touch Conflict"
        case "primary-control-too-high", "primary-control-too-central", "primary-control-out-of-reach":
            return plural ? "\(count) Reach Issues" : "Thumb Reach"
        case "portrait-primary-action-distribution", "portrait-dead-space":
            return plural ? "\(count) Portrait Issues" : "Portrait Layout"
        case "layout-displacement":
            return plural ? "\(count) Displaced" : "Displaced Control"
        case "small-control":
            return plural ? "\(count) Small Controls" : "Small Control"
        case "edge-hugging-control":
            return plural ? "\(count) Edge Issues" : "Edge Spacing"
        case "underused-bottom-space", "low-vertical-coverage", "low-horizontal-coverage":
            return plural ? "\(count) Coverage Issues" : "Canvas Coverage"
        default:
            return plural ? "\(count) Layout Issues" : "Layout Issue"
        }
    }

    private func layoutQualityBadgeImage(_ report: GamepadLayoutQualityReport) -> String {
        if report.hasErrors { return "xmark.octagon.fill" }
        if report.hasWarnings { return "exclamationmark.triangle.fill" }
        return "checkmark.circle.fill"
    }

    private var editorSaveAndSyncStatusText: String {
        if hasPendingExternalEditorCommit { return "Saving locally…" }
        if let editorDeliveryStatusText, !editorDeliveryStatusText.isEmpty { return editorDeliveryStatusText }
        return connectedDeviceInfo == nil ? "Saved locally · iPhone offline" : "Saved locally · Live sync"
    }

    private var editorSaveAndSyncStatusImage: String {
        if hasPendingExternalEditorCommit { return "arrow.triangle.2.circlepath" }
        return connectedDeviceInfo == nil ? "iphone.slash" : "checkmark.icloud"
    }

    private func announceEditor(_ message: String) {
            ...
            HStack(spacing: 0) {
                if isConfigurationSidebarVisible {
                    ...
                    GamepadEditorResizeHandle(
                        ...
                        accessibilityValue: "\(Int(sidebarWidths.configuration.rounded())) points wide",
                        onAccessibilityAdjust: adjustConfigurationSidebarWidth,
                ...
                if isInspectorSidebarVisible {
                    GamepadEditorResizeHandle(
                        ...
                        accessibilityValue: "\(Int(sidebarWidths.inspector.rounded())) points wide",
                        onAccessibilityAdjust: adjustInspectorSidebarWidth,
                        onDragChanged: { value in
    ...
    private var configurationSidebar: some View {
        VStack(alignment: .leading, spacing: 0) {
            VStack(alignment: .leading, spacing: Geist.Spacing.s3) {
                ...
                Text(configurationSidebarTab == .setups ? "Create and organize keypad setups." : selectedProfile?.name ?? "Current Setup")
                    .geistTypography(.copy13)
                    .foregroundStyle(Geist.color(.gray900, scheme: colorScheme))
                    .lineLimit(configurationSidebarTab == .setups ? 2 : 1)

                Picker("Sidebar", selection: $configurationSidebarTab) {
                    ForEach(GamepadConfigurationSidebarTab.allCases) { tab in
                ...
                ScrollViewReader { proxy in
                    ScrollView(.vertical, showsIndicators: true) {
                        LazyVStack(spacing: Geist.Spacing.s2) {
                            if filteredSetupProfiles.isEmpty {
                                VStack(spacing: Geist.Spacing.s2) {
                                    Image(systemName: "magnifyingglass")
                                        .font(.system(size: 20, weight: .semibold))
                                        .foregroundStyle(Geist.color(.gray900, scheme: colorScheme))
                                    Text("No setups match “\(setupSearchText)”.")
                                        .geistTypography(.copy13)
                                        .foregroundStyle(Geist.color(.gray900, scheme: colorScheme))
                                        .multilineTextAlignment(.center)
                                    Button("Clear Search") { setupSearchText = "" }
                                        .geistButtonStyle(.secondary, size: .small)
                                }
                                .padding(Geist.Spacing.s4)
                                .frame(maxWidth: .infinity)
                            } else {
                                ForEach(filteredSetupProfiles) { profile in
                                    profileNavigationRow(profile)
                                        .id(profile.id)
                                }
                            }
                        }
                    ...
                    .onAppear {
                        scrollToSelectedSetup(in: proxy, clearsHiddenFilter: true, animated: false)
                    }
                    .onChange(of: selectedProfileID) { _, _ in
                        scrollToSelectedSetup(in: proxy, clearsHiddenFilter: true)
                    }
                    .onChange(of: profiles.map(\.id)) { _, _ in
                        scrollToSelectedSetup(in: proxy, clearsHiddenFilter: false)
                    }
                    .onChange(of: setupSearchText) { _, _ in
                        scrollToSelectedSetup(in: proxy, clearsHiddenFilter: false)
                    }
                    .onMoveCommand(perform: moveSetupSelection)
                }
    ...
    private var setupSearchField: some View {
        HStack(spacing: Geist.Spacing.s2) {
            Image(systemName: "magnifyingglass")
                .foregroundStyle(Geist.color(.gray900, scheme: colorScheme))
            TextField("Search setups", text: $setupSearchText)
                .textFieldStyle(.plain)
            if !setupSearchText.isEmpty {
                Button { setupSearchText = "" } label: {
                    Image(systemName: "xmark.circle.fill")
                }
                .buttonStyle(.plain)
                .accessibilityLabel("Clear setup search")
            }
        }
        .padding(.horizontal, Geist.Spacing.s2)
        .frame(height: Geist.Spacing.s8)
    ...
    private var filteredSetupProfiles: [GamepadConfigurationProfile] {
        let query = setupSearchText.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !query.isEmpty else { return profiles }
        return profiles.filter { profile in
            [
                profile.name,
                profile.launchTarget?.displayName,
                profile.launchTarget?.detailText
            ]
            .compactMap { $0 }
    ...
    private func moveSetupSelection(_ direction: MoveCommandDirection) {
        let visibleProfiles = filteredSetupProfiles
        guard !visibleProfiles.isEmpty else { return }
        let currentIndex = visibleProfiles.firstIndex { $0.id == selectedProfileID } ?? 0
        let delta: Int
        switch direction {
        case .up, .left:
            delta = -1
        ...
        let nextIndex = min(max(currentIndex + delta, 0), visibleProfiles.count - 1)
        guard nextIndex != currentIndex else { return }
        let profile = visibleProfiles[nextIndex]
        selectProfile(profile, expandsDetails: false)
        announceEditor("Selected \(profile.name) keypad setup.")
    }

    private func scrollToSelectedSetup(
        in proxy: ScrollViewProxy,
        clearsHiddenFilter: Bool,
        animated: Bool = true
    ) {
        let selectedID = selectedProfileID
        let activeSetupIsVisible = filteredSetupProfiles.contains { $0.id == selectedID }
        if !activeSetupIsVisible {
            guard clearsHiddenFilter else { return }
            setupSearchText = ""
        }
    ...
    private func profileNavigationRow(_ profile: GamepadConfigurationProfile) -> some View {
        ...
        return HStack(spacing: 0) {
            Button {
                ...
                HStack(spacing: Geist.Spacing.s2) {
                    Image(systemName: profile.launchTarget == nil ? "rectangle" : "app.badge.fill")
                    ...
                    if isActive {
                        Image(systemName: accessibilityDifferentiateWithoutColor ? "checkmark.circle.fill" : "circle.fill")
                            .help("Active")
                    }
                }
                .foregroundStyle(Geist.color(.gray1000, scheme: colorScheme))
            ...
            .buttonStyle(.plain)
            .accessibilityLabel("\(profile.name) keypad setup")
            .accessibilityValue(profileAccessibilityState(profile))
            .accessibilityAddTraits(isActive ? .isSelected : [])
            .accessibilityAction(named: Text("Rename")) { beginRenamingProfile(profile) }
            .accessibilityAction(named: Text("Duplicate")) { duplicateProfile(profile) }
            .accessibilityAction(named: Text("Make Default")) {
                if profile.id != selectedProfileID { selectProfile(profile, expandsDetails: false) }
                setSelectedProfileAsDefault()
            }
            .accessibilityAction(named: Text("Move Earlier")) { moveProfile(profile, by: -1) }
            .accessibilityAction(named: Text("Move Later")) { moveProfile(profile, by: 1) }
        ...
        .overlay(
            RoundedRectangle(cornerRadius: Geist.Radius.sm, style: .continuous)
                .stroke(
                    profileSelectionStroke(isActive: isActive, isSelected: isSelected),
                    lineWidth: accessibilityContrast == .increased || isSelected ? 2 : 1
                )
        )
        .contextMenu { profileContextMenu(for: profile) }
        .onDrag { profileDragItemProvider(for: profile) }
        .onDrop(
            of: [Self.profileDragUTType],
            delegate: GamepadProfileDropDelegate(
    ...
    private func profileAccessibilityState(_ profile: GamepadConfigurationProfile) -> String {
        var states: [String] = []
        if profile.id == selectedProfileID { states.append("Active") }
        if profile.id == defaultProfileID { states.append("Default") }
        if profile.launchTarget != nil { states.append("Application attached") }
        return states.isEmpty ? "Not active" : states.joined(separator: ", ")
    }
    ...
    @ViewBuilder
    private func importMenu(showsTitle: Bool) -> some View {
        if onImportProfiles != nil {
            Menu {
                Button {
                    importsProfilesAsCopies = false
                    isProfileImporterPresented = true
                } label: {
                    Label("Import and Replace Matching…", systemImage: "square.and.arrow.down")
                }

                Button {
                    importsProfilesAsCopies = true
                    isProfileImporterPresented = true
                } label: {
                    Label("Import as New Setups…", systemImage: "plus.square.on.square")
                }
            } label: {
                if showsTitle {
    ...
    private func handleProfileImport(_ result: Result<[URL], Error>) {
        ...
        case .success(let urls):
            guard let url = urls.first, let onImportProfiles else { return }
            ...
            do {
                ...
                profileImportNotice = try onImportProfiles(
                    ...
                    importsProfilesAsCopies
                )
            } catch {
                profileImportNotice = error.localizedDescription
            }
    ...
    private func exportMenu(showsTitle: Bool) -> some View {
        if onExportProfiles != nil {
            Menu {
                Button {
                    exportProfiles(profileID: selectedProfileID)
                } label: {
                    Label("Export Current Setup…", systemImage: "square.and.arrow.up")
                }

                Button {
                    exportProfiles(profileID: nil)
                } label: {
                    Label("Export All Setups…", systemImage: "square.and.arrow.up.on.square")
                }
            } label: {
                if showsTitle {
        ...
        VStack(alignment: .leading, spacing: Geist.Spacing.s2) {
            ...
            if !componentListItems.isEmpty {
                HStack(spacing: Geist.Spacing.s2) {
                    Image(systemName: "magnifyingglass")
                        .foregroundStyle(Geist.color(.gray900, scheme: colorScheme))
                    TextField("Search layers", text: $layerSearchText)
                        .textFieldStyle(.plain)
                    if !layerSearchText.isEmpty {
                        Button { layerSearchText = "" } label: {
                            Image(systemName: "xmark.circle.fill")
                        }
                        .buttonStyle(.plain)
                        .accessibilityLabel("Clear layer search")
                    }
                }
                .padding(.horizontal, Geist.Spacing.s2)
                .frame(height: Geist.Spacing.s8)
                .background(Geist.color(.gray100, scheme: colorScheme), in: RoundedRectangle(cornerRadius: Geist.Radius.sm, style: .continuous))
                .overlay(RoundedRectangle(cornerRadius: Geist.Radius.sm, style: .continuous).stroke(Geist.color(.grayAlpha300, scheme: colorScheme), lineWidth: 1))
                .padding(.horizontal, Geist.Spacing.s2)
            }

            if componentListItems.isEmpty {
                emptyComponentsMessage
            } else if filteredLayerListItems.isEmpty {
                Text("No layers match “\(layerSearchText)”.")
                    .geistTypography(.copy13)
                    .foregroundStyle(Geist.color(.gray900, scheme: colorScheme))
                    .padding(Geist.Spacing.s3)
            } else {
                LazyVStack(spacing: Geist.Spacing.s1) {
                    ForEach(filteredLayerListItems) { listItem in
                        switch listItem {
    ...
    private func layerGroupRow(_ group: GamepadEditorLayerGroupItem) -> some View {
        ...
        .overlay(
            RoundedRectangle(cornerRadius: Geist.Radius.sm, style: .continuous)
                .stroke(
                    isSelected ? Geist.color(.blue700, scheme: colorScheme) : Color.clear,
                    lineWidth: accessibilityContrast == .increased ? 2.5 : 1.5
                )
        )
        ...
        .onDrop(
            of: [Self.layerDragUTType],
            delegate: GamepadLayerDropDelegate(
                targetIdentities: group.childIdentities,
                draggingIdentities: $draggingLayerControlIDs,
                onMove: moveDraggedLayers,
    ...
    private func componentRow(_ item: GamepadEditorComponentItem, indentation: CGFloat = 0) -> some View {
        ...
        return HStack(spacing: Geist.Spacing.s1) {
            ...
            .accessibilityValue("\(isSelected ? "Selected, " : "")\(item.subtitle), \(item.isHidden ? "Hidden, " : "Visible, ")\(item.isLocationLocked ? "Locked" : "Unlocked")")
            .accessibilityAddTraits(isSelected ? .isSelected : [])
            .accessibilityAction(named: Text(item.isLocationLocked ? "Unlock Location" : "Lock Location")) {
                toggleComponentLock(item.identity)
            }
            .accessibilityAction(named: Text(item.isHidden ? "Show" : "Hide")) {
            ...
            .accessibilityAction(named: Text("Bring Forward")) { bringLayerForward(item.identity) }
            .accessibilityAction(named: Text("Send Backward")) { sendLayerBackward(item.identity) }
        ...
        .onDrop(
            of: [Self.layerDragUTType],
            delegate: GamepadLayerDropDelegate(
                targetIdentities: [item.identity],
                draggingIdentities: $draggingLayerControlIDs,
                onMove: moveDraggedLayers,
                onDropEnded: finishLayerDrag
            )
    ...
    private var blankSetupCanvasCard: some View {
        GamepadEditorBlankSetupCard(
            ...
            onApplyTemplate: { template in
                applyTemplateToCurrentSetup(template)
                completeFirstKeypadOnboarding()
            },
            onShowTour: {
    ...
    private var canvasStage: some View {
        GeometryReader { proxy in
            ...
            let fitWidth = max(120, viewportWidth - (Self.canvasFitInset * 2))
            let fitHeight = max(120, viewportHeight - (Self.canvasFitInset * 2))
            let fitScale = max(
                0.001,
                min(
    ...
    private func canvasViewport(
        ...
        let selectionAnchorPoint = canvasSelectionBounds.map { bounds in
            CGPoint(
                x: screenDisplayRect.minX + bounds.midX * displayScale,
                y: screenDisplayRect.minY + bounds.midY * displayScale
            )
        }
        ...
        return ScrollViewReader { scrollProxy in
            ScrollView([.horizontal, .vertical], showsIndicators: true) {
                ZStack {
                    ...
                    if let selectionAnchorPoint {
                        Color.clear
                            .frame(width: 2, height: 2)
                            .position(selectionAnchorPoint)
                            .id("canvas-selection-anchor")
                            .allowsHitTesting(false)
                            .accessibilityHidden(true)
                    }
                ...
                .shadow(
                    color: Color.black.opacity(colorScheme == .dark ? 0.72 : 0.16),
                    radius: colorScheme == .dark ? 22 : 14,
                    x: 0,
            ...
            .onChange(of: canvasScrollRequestID) { _, requestID in
                guard requestID != nil, selectionAnchorPoint != nil else { return }
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.05) {
                    withAnimation(accessibilityReduceMotion ? nil : .easeInOut(duration: 0.18)) {
                        scrollProxy.scrollTo("canvas-selection-anchor", anchor: .center)
                    }
                }
            }
        }
        ...
        .overlay(alignment: .bottom) {
            if let activeLayoutQualityIssue {
                layoutIssueRepairBanner(activeLayoutQualityIssue)
                    .padding(Geist.Spacing.s4)
            }
        }
        ...
        .background(canvasPasteboardColor)
    ...
    private func layoutIssueRepairBanner(_ issue: GamepadLayoutIssue) -> some View {
        HStack(spacing: Geist.Spacing.s3) {
            Image(systemName: issue.severity == .error ? "xmark.octagon.fill" : "exclamationmark.triangle.fill")
                .font(.system(size: 18, weight: .semibold))
                .foregroundStyle(Geist.color(issue.severity == .error ? .red900 : .amber900, scheme: colorScheme))

            VStack(alignment: .leading, spacing: 2) {
                Text(issue.severity == .error ? "Layout Error" : "Layout Suggestion")
                    .geistTypography(.heading14)
                Text(issue.message)
                    .geistTypography(.copy13)
                    .foregroundStyle(Geist.color(.gray900, scheme: colorScheme))
            ...
            if let recommendedRepair = issue.suggestedRepairs.first {
                Button(recommendedRepair.displayName) {
                    applyLayoutRepair(recommendedRepair, to: issue)
                }
                .geistButtonStyle(.primary, size: .small)
            }

            if issue.suggestedRepairs.count > 1 {
                Menu {
                    ForEach(issue.suggestedRepairs.dropFirst()) { repair in
                        Button {
                            applyLayoutRepair(repair, to: issue)
                        } label: {
                            Label(repair.displayName, systemImage: repair.systemImage)
                ...
                .menuStyle(.button)
                .geistButtonStyle(.secondary, size: .small)
                .accessibilityLabel("More layout repairs")
            }
            ...
            .buttonStyle(.plain)
            .accessibilityLabel("Dismiss layout suggestion")
        }
        .padding(Geist.Spacing.s3)
        .frame(maxWidth: 680)
        .background(Geist.color(.background100, scheme: colorScheme).opacity(0.97), in: RoundedRectangle(cornerRadius: Geist.Radius.md, style: .continuous))
        .overlay(
            RoundedRectangle(cornerRadius: Geist.Radius.md, style: .continuous)
                .stroke(
                    Geist.color(issue.severity == .error ? .red700 : .amber700, scheme: colorScheme),
                    lineWidth: accessibilityContrast == .increased ? 2.5 : 1.5
                )
        )
        .shadow(color: Color.black.opacity(colorScheme == .dark ? 0.38 : 0.14), radius: 12, y: 5)
        .accessibilityElement(children: .contain)
        .accessibilityLabel("\(issue.severity.rawValue.capitalized): \(issue.message)")
        .accessibilityFocused($isLayoutIssueAccessibilityFocused)
    }
    ...
    private var shortcutReferenceSheet: some View {
        VStack(alignment: .leading, spacing: Geist.Spacing.s4) {
            HStack {
                VStack(alignment: .leading, spacing: Geist.Spacing.s1) {
                    Text("Keypad Editor Shortcuts")
                        .geistTypography(.heading24)
                    Text("The same commands are available from the command bar and context menus.")
                        .geistTypography(.copy13)
                        .foregroundStyle(Geist.color(.gray900, scheme: colorScheme))
                }
                Spacer()
                Button("Done") { isShortcutReferencePresented = false }
                    .geistButtonStyle(.secondary, size: .small)
            }
            ...
            ScrollView {
                LazyVGrid(columns: [GridItem(.adaptive(minimum: 245), spacing: Geist.Spacing.s3)], alignment: .leading, spacing: Geist.Spacing.s3) {
                    shortcutGroup("Selection", shortcuts: [
                        ("⌘A", "Select all visible controls"),
                        ("⇧ Click", "Add a control to the selection"),
                        ("Drag", "Marquee-select an area"),
                    ...
                    shortcutGroup("Tools & Movement", shortcuts: [
                        ("V", "Select tool"),
                        ("R", "Draw rectangle"),
                        ("O", "Draw ellipse"),
                    ...
                    shortcutGroup("View", shortcuts: [
                        ("⌘0", "Fit canvas"),
                        ("⌘+ / ⌘−", "Zoom in or out"),
                        ("⌥⌘0", "Focus canvas or restore sidebars"),
                        ("⌥⌘1", "Toggle Setups sidebar"),
                        ("⌥⌘2", "Toggle Inspector"),
                        ("⌘/", "Show this reference")
                    ])
                }
            ...
            ForEach(Array(shortcuts.enumerated()), id: \.offset) { _, shortcut in
                HStack(alignment: .firstTextBaseline, spacing: Geist.Spacing.s3) {
                    Text(shortcut.0)
                        ...
                        .padding(.horizontal, Geist.Spacing.s2)
                        .padding(.vertical, Geist.Spacing.s1)
                        .background(Geist.color(.gray200, scheme: colorScheme), in: RoundedRectangle(cornerRadius: Geist.Radius.sm, style: .continuous))
    ...
    private var orientationComparisonSheet: some View {
        VStack(alignment: .leading, spacing: Geist.Spacing.s4) {
            HStack {
                VStack(alignment: .leading, spacing: Geist.Spacing.s1) {
                    Text("Compare Orientations")
                        .geistTypography(.heading24)
                    Text("Review both saved variants before choosing one to edit.")
                        .geistTypography(.copy13)
                        .foregroundStyle(Geist.color(.gray900, scheme: colorScheme))
                }
                Spacer()
                Button("Done") { isOrientationComparisonPresented = false }
                    .geistButtonStyle(.secondary, size: .small)
            }

            HStack(alignment: .top, spacing: Geist.Spacing.s4) {
                orientationComparisonCard(.landscape)
                orientationComparisonCard(.portrait)
            }
            .frame(maxWidth: .infinity, maxHeight: .infinity)
        }
        .padding(Geist.Spacing.s6)
    ...
    private func orientationComparisonCard(_ orientation: GamepadEditorDeviceOrientation) -> some View {
        let hasVariant = hasSavedOrientation(orientation)
        let previewCustomization = orientation == selectedProfileOrientation ? customization : (selectedProfile?.customization(for: orientation) ?? customization)
        let report = previewCustomization.layoutQualityReport(profileName: selectedProfile?.name)

        return VStack(alignment: .leading, spacing: Geist.Spacing.s3) {
            HStack {
                Label(orientation.displayName, systemImage: orientation == .landscape ? "rectangle" : "rectangle.portrait")
                    .geistTypography(.heading16)
                Spacer()
                Label(hasVariant ? "Customized" : "Not created", systemImage: hasVariant ? "checkmark.circle.fill" : "circle.dashed")
                    .geistTypography(.label12)
                    .foregroundStyle(Geist.color(hasVariant ? .blue900 : .gray900, scheme: colorScheme))
            }

            if hasVariant {
                GamepadCustomizationPreview(customization: previewCustomization)
                    .environment(\.colorScheme, activeKeypadColorScheme)
                    .aspectRatio(orientation == .landscape ? 2.05 : 0.49, contentMode: .fit)
                    .frame(maxWidth: .infinity, maxHeight: .infinity)
                    .background(Geist.color(.background200, scheme: colorScheme), in: RoundedRectangle(cornerRadius: Geist.Radius.md, style: .continuous))

                Label(
                    report.issues.isEmpty ? "No layout suggestions" : "\(report.summary.errorCount) errors · \(report.summary.warningCount) warnings",
                    systemImage: report.hasErrors ? "xmark.octagon.fill" : (report.hasWarnings ? "exclamationmark.triangle.fill" : "checkmark.circle.fill")
                )
                .geistTypography(.label13)
            } else {
                VStack(spacing: Geist.Spacing.s3) {
                    ...
                    HStack(spacing: Geist.Spacing.s2) {
                        Button("Copy") {
                            transferOrientation(from: orientation == .landscape ? .portrait : .landscape, to: orientation, automaticallyArranged: false)
                        }
                        .geistButtonStyle(.secondary, size: .small)
                        Button("Auto Arrange") {
                            transferOrientation(from: orientation == .landscape ? .portrait : .landscape, to: orientation, automaticallyArranged: true)
                        }
                        .geistButtonStyle(.primary, size: .small)
                    }
                }
            ...
            Button("Edit \(orientation.displayName)") {
                isOrientationComparisonPresented = false
                setDeviceFrame(GamepadEditorDeviceFrame(spec: activeDeviceFrame.spec, orientation: orientation))
            }
            .geistButtonStyle(.primary, size: .small)
            .disabled(!hasVariant)
        ...
        .background(Geist.color(.gray100, scheme: colorScheme), in: RoundedRectangle(cornerRadius: Geist.Radius.lg, style: .continuous))
        .overlay(RoundedRectangle(cornerRadius: Geist.Radius.lg, style: .continuous).stroke(Geist.color(.grayAlpha400, scheme: colorScheme), lineWidth: 1))
    }

    private var resourceLibrarySheet: some View {
        VStack(alignment: .leading, spacing: Geist.Spacing.s4) {
            HStack(alignment: .firstTextBaseline) {
                VStack(alignment: .leading, spacing: Geist.Spacing.s1) {
                    Text("Keypad Resources")
                        .geistTypography(.heading24)
                    Text("Reusable styles and embedded assets are saved with this keypad setup.")
                        .geistTypography(.copy13)
                        .foregroundStyle(Geist.color(.gray900, scheme: colorScheme))
                }
                Spacer()
                Button("Done") { isResourceLibraryPresented = false }
                    .geistButtonStyle(.secondary, size: .small)
            }

            GeistSegmentedPicker(
                title: "Resource type",
                options: GamepadResourceLibraryTab.allCases,
                selection: $resourceLibraryTab,
                label: { $0.title }
            )
            .frame(maxWidth: 320)
        ...
        .background(Geist.color(.background100, scheme: colorScheme))
        .onAppear {
            if selectedStyleTokenID == nil {
                selectedStyleTokenID = customization.styleLibrary.normalized.styles.first?.id
            }
            if selectedAssetID == nil {
                selectedAssetID = customization.assetLibrary.normalized.assets.first?.id
            }
        }
    }

    private var styleLibraryContent: some View {
        HStack(alignment: .top, spacing: Geist.Spacing.s4) {
            VStack(alignment: .leading, spacing: Geist.Spacing.s3) {
                HStack {
                    Text("Named Styles")
                    ...
                    .geistButtonStyle(.secondary, size: .small)
                    .disabled(!selectedControlIsEditable)
                }

                ScrollView {
                    LazyVStack(spacing: Geist.Spacing.s2) {
                        ForEach(customization.styleLibrary.normalized.styles) { style in
                            Button {
                                selectedStyleTokenID = style.id
                            } label: {
                                HStack(spacing: Geist.Spacing.s3) {
                                    GamepadFillPreview(
                                        shape: RoundedRectangle(cornerRadius: Geist.Radius.sm, style: .continuous),
                                        fillStyle: style.visualStyle.normal.fillStyle ?? .solid(GamepadRGBAColor(red: 0.5, green: 0.5, blue: 0.5))
                                    )
                                    .frame(width: 34, height: 28)
                                    .overlay(RoundedRectangle(cornerRadius: Geist.Radius.sm).stroke(Geist.color(.grayAlpha400, scheme: colorScheme), lineWidth: 1))
                                    VStack(alignment: .leading, spacing: 1) {
                                        Text(style.name)
                                            .geistTypography(.label13)
                                        Text(style.id)
                                ...
                                .padding(Geist.Spacing.s2)
                                .background(
                                    selectedStyleTokenID == style.id ? Geist.color(.blue100, scheme: colorScheme) : Geist.color(.gray100, scheme: colorScheme),
                                    in: RoundedRectangle(cornerRadius: Geist.Radius.sm, style: .continuous)
                                )
                            }
                            .buttonStyle(.plain)
                ...
                HStack(spacing: Geist.Spacing.s2) {
                    Button("Import…") { isStyleLibraryImporterPresented = true }
                        .geistButtonStyle(.tertiary, size: .small)
                    Button("Export…") {
                        styleLibraryExportDocument = GamepadStyleLibraryJSONDocument(library: customization.styleLibrary.normalized)
                        isStyleLibraryExporterPresented = true
                    }
                    .geistButtonStyle(.tertiary, size: .small)
                    .disabled(customization.styleLibrary.normalized.isEmpty)
                }
            }
            .frame(width: 280)
            ...
            if let selectedStyleToken {
                VStack(alignment: .leading, spacing: Geist.Spacing.s4) {
                    ...
                    VStack(alignment: .leading, spacing: Geist.Spacing.s1) {
                        Text("Name")
                            .geistTypography(.label12)
                        TextField("Style name", text: selectedStyleNameBinding)
                            .geistInput(size: .small)
                    }
                    ...
                    GamepadFillPreview(
                        ...
                        fillStyle: selectedStyleToken.visualStyle.normal.fillStyle ?? .solid(GamepadRGBAColor(red: 0.5, green: 0.5, blue: 0.5))
                    )
                    .frame(height: 92)

                    Text("Select an element, edit its appearance in the inspector, then update this style. Every element using the style updates immediately.")
                        .geistTypography(.copy13)
                        .foregroundStyle(Geist.color(.gray900, scheme: colorScheme))
                        .fixedSize(horizontal: false, vertical: true)
                    ...
                    HStack(spacing: Geist.Spacing.s2) {
                        Button("Apply to Selection") {
                            styleIDBinding(for: selectedControlID).wrappedValue = selectedStyleToken.id
                        }
                        .geistButtonStyle(.primary, size: .small)
                        .disabled(!selectedControlIsEditable)

                        Button("Update from Selection") {
                            updateSelectedStyleFromElement()
                        }
                        .geistButtonStyle(.secondary, size: .small)
                        .disabled(!selectedControlIsEditable)
                    }
                ...
                VStack(spacing: Geist.Spacing.s3) {
                    Image(systemName: "paintbrush")
                        .font(.system(size: 34))
                    Text("No named style selected")
                        .geistTypography(.heading16)
                    Text("Create one from the selected keypad element or import a style library.")
                        .geistTypography(.copy13)
                        .foregroundStyle(Geist.color(.gray900, scheme: colorScheme))
                }
                .frame(maxWidth: .infinity, maxHeight: .infinity)
    ...
    private var assetLibraryContent: some View {
        HStack(alignment: .top, spacing: Geist.Spacing.s4) {
            VStack(alignment: .leading, spacing: Geist.Spacing.s3) {
                HStack {
                    Text("Embedded Assets")
                        .geistTypography(.heading16)
                    Spacer()
                    Button {
                        isAssetImporterPresented = true
                    } label: {
                ...
                ScrollView {
                    LazyVStack(spacing: Geist.Spacing.s2) {
                        ForEach(customization.assetLibrary.normalized.assets) { asset in
                            Button {
                                ...
                                HStack(spacing: Geist.Spacing.s3) {
                                    ...
                                    VStack(alignment: .leading, spacing: 1) {
                                        Text(asset.name)
                                            .geistTypography(.label13)
                                        Text("\(asset.role.rawValue.capitalized) · \(ByteCountFormatter.string(fromByteCount: Int64(asset.byteCount), countStyle: .file))")
                                            .geistTypography(.label12)
                                            .foregroundStyle(Geist.color(.gray900, scheme: colorScheme))
                                    }
                                    Spacer()
                                ...
                                .padding(Geist.Spacing.s2)
                                .background(
                                    selectedAssetID == asset.id ? Geist.color(.blue100, scheme: colorScheme) : Geist.color(.gray100, scheme: colorScheme),
                                    in: RoundedRectangle(cornerRadius: Geist.Radius.sm, style: .continuous)
                                )
                            }
                            .buttonStyle(.plain)
            ...
            if let selectedAsset {
                VStack(alignment: .leading, spacing: Geist.Spacing.s4) {
                    assetThumbnail(selectedAsset)
                        .frame(width: 96, height: 96)

                    VStack(alignment: .leading, spacing: Geist.Spacing.s1) {
                        Text("Name")
                            .geistTypography(.label12)
                        TextField("Asset name", text: selectedAssetNameBinding)
                            .geistInput(size: .small)
                    }

                    Picker("Role", selection: selectedAssetRoleBinding) {
                        ForEach(GamepadAssetRole.allCases) { role in
                            Text(role.rawValue.capitalized).tag(role)
                        }
                    }

                    Group {
                        Text(selectedAsset.fileName ?? "No filename")
                        Text(selectedAsset.contentType)
                        Text("\(selectedAsset.byteCount) bytes")
                        Text(selectedAsset.id)
                    }
                    .geistTypography(.label13Mono)
                    .textSelection(.enabled)

                    Text("Library assets travel with the keypad. To immediately use an image as a fill, open the element or background Fill editor.")
                        .geistTypography(.copy13)
                        .foregroundStyle(Geist.color(.gray900, scheme: colorScheme))
                        .fixedSize(horizontal: false, vertical: true)

                    HStack(spacing: Geist.Spacing.s2) {
                        Button("Copy ID") { copyResourceID(selectedAsset.id) }
                            .geistButtonStyle(.secondary, size: .small)
                        Button("Delete Asset", role: .destructive) { deleteSelectedAsset() }
                            .geistButtonStyle(.error, size: .small)
                    }
                    Spacer()
                }
                ...
                VStack(spacing: Geist.Spacing.s3) {
                    ...
                    Text("No asset selected")
                        .geistTypography(.heading16)
                    Text("Import an image or reference file to store it with this keypad.")
                        .geistTypography(.copy13)
                        .foregroundStyle(Geist.color(.gray900, scheme: colorScheme))
                }
                .frame(maxWidth: .infinity, maxHeight: .infinity)
    ...
    private var selectedStyleToken: GamepadStyleToken? {
        customization.styleLibrary.style(id: selectedStyleTokenID)
    }

    private var selectedAsset: GamepadAsset? {
        customization.assetLibrary.asset(id: selectedAssetID)
    }

    private var selectedStyleNameBinding: Binding<String> {
        Binding(
            get: { selectedStyleToken?.name ?? "" },
            set: { name in
                guard let selectedStyleTokenID else { return }
                let trimmedName = name.trimmingCharacters(in: .whitespacesAndNewlines)
                guard !trimmedName.isEmpty else { return }
                update(actionName: "Rename Style") { next in
                    guard let index = next.styleLibrary.styles.firstIndex(where: { $0.id == selectedStyleTokenID }) else { return }
                    next.styleLibrary.styles[index].name = trimmedName
                }
            }
        )
    }

    private var selectedAssetNameBinding: Binding<String> {
        Binding(
            ...
            set: { name in
                ...
                let trimmedName = name.trimmingCharacters(in: .whitespacesAndNewlines)
                guard !trimmedName.isEmpty else { return }
                update(actionName: "Rename Asset") { next in
                    guard let index = next.assetLibrary.assets.firstIndex(where: { $0.id == selectedAssetID }) else { return }
                    next.assetLibrary.assets[index].name = trimmedName
                }
            }
        )
    }

    private var selectedAssetRoleBinding: Binding<GamepadAssetRole> {
        Binding(
            get: { selectedAsset?.role ?? .reference },
            set: { role in
                guard let selectedAssetID else { return }
                update(actionName: "Change Asset Role") { next in
                    guard let index = next.assetLibrary.assets.firstIndex(where: { $0.id == selectedAssetID }) else { return }
                    next.assetLibrary.assets[index].role = role
                }
            }
        )
    }

    private func styleSnapshotFromSelectedElement() -> GamepadControlVisualStyle {
        let layout = selectedLayoutCustomization(for: selectedControlID)
        let baseStyle = customization.styleLibrary.style(id: layout.styleID)?.visualStyle ?? .empty
        let localStyle = layout.visualStyle ?? .empty
        var visualStyle = GamepadControlVisualStyle(
            normal: localStyle.normal.merged(over: baseStyle.normal),
            pressed: localStyle.pressed.map { $0.merged(over: baseStyle.pressed ?? .empty) } ?? baseStyle.pressed,
            active: localStyle.active.map { $0.merged(over: baseStyle.active ?? .empty) } ?? baseStyle.active,
            disabled: localStyle.disabled.map { $0.merged(over: baseStyle.disabled ?? .empty) } ?? baseStyle.disabled,
            icon: localStyle.icon ?? baseStyle.icon,
            hapticStyle: localStyle.hapticStyle ?? baseStyle.hapticStyle,
            hapticFeedback: localStyle.hapticFeedback ?? baseStyle.hapticFeedback
        )
        if visualStyle.normal.fillStyle == nil {
            visualStyle.normal.fillStyle = selectedFillStyleValue(for: selectedControlID, scheme: activeKeypadColorScheme)
        }
        if visualStyle.icon == nil { visualStyle.icon = layout.icon }
        if visualStyle.hapticFeedback == nil, layout.hapticFeedback != nil {
            visualStyle.hapticFeedback = layout.resolvedHapticFeedback
        }
        return visualStyle.normalized ?? GamepadControlVisualStyle(
            normal: GamepadControlStateStyle(fillStyle: selectedFillStyleValue(for: selectedControlID, scheme: activeKeypadColorScheme))
        )
    }

    private func createStyleFromSelectedElement() {
        guard selectedControlIsEditable else { return }
        let name = "Style \(customization.styleLibrary.normalized.styles.count + 1)"
        var id = GamepadStyleToken.normalizedIdentifier(name.lowercased())
        var suffix = 2
        while customization.styleLibrary.style(id: id) != nil {
            id = "\(GamepadStyleToken.normalizedIdentifier(name.lowercased()))-\(suffix)"
            suffix += 1
        }
        let token = GamepadStyleToken(id: id, name: name, visualStyle: styleSnapshotFromSelectedElement())
        update(actionName: "Create Style") { next in
            next.styleLibrary.styles.append(token)
        }
        selectedStyleTokenID = id
        styleIDBinding(for: selectedControlID).wrappedValue = id
    }

    private func updateSelectedStyleFromElement() {
        guard let selectedStyleTokenID, selectedControlIsEditable else { return }
        let snapshot = styleSnapshotFromSelectedElement()
        update(actionName: "Update Style") { next in
            guard let index = next.styleLibrary.styles.firstIndex(where: { $0.id == selectedStyleTokenID }) else { return }
            next.styleLibrary.styles[index].visualStyle = snapshot
        }
    }
    ...
    private func deleteSelectedStyle() {
        ...
        update(actionName: "Delete Style") { next in
            ...
            for index in next.customButtons.indices where next.customButtons[index].layout.styleID == selectedStyleTokenID {
                next.customButtons[index].layout.styleID = nil
            }
            if next.topBarActivationRegion.styleID == selectedStyleTokenID {
                next.topBarActivationRegion.styleID = nil
            }
            for item in next.controlBarItems {
                var appearance = next.controlBarItemCustomization(for: item)
                if appearance.styleID == selectedStyleTokenID {
                    appearance.styleID = nil
                    next.setControlBarItemCustomization(appearance, for: item)
                }
            }
        }
        self.selectedStyleTokenID = customization.styleLibrary.normalized.styles.first { $0.id != selectedStyleTokenID }?.id
    }

    private func deleteSelectedAsset() {
        guard let selectedAssetID else { return }
        update(actionName: "Delete Asset") { next in
            next.assetLibrary.assets.removeAll { $0.id == selectedAssetID }
        }
        self.selectedAssetID = customization.assetLibrary.normalized.assets.first { $0.id != selectedAssetID }?.id
    }

    private func copyResourceID(_ id: String) {
    ...
    @ViewBuilder
    private func assetThumbnail(_ asset: GamepadAsset) -> some View {
        if let data = asset.data, let image = NSImage(data: data) {
            Image(nsImage: image)
                .resizable()
                .scaledToFit()
                .frame(width: 42, height: 42)
                .background(Geist.color(.gray200, scheme: colorScheme), in: RoundedRectangle(cornerRadius: Geist.Radius.sm))
        } else {
    ...
    private func handleStyleLibraryImport(_ result: Result<[URL], Error>) {
        handleResourceFileResult(result) { url, data in
            let imported = try JSONDecoder().decode(GamepadStyleLibrary.self, from: data).normalized
            update(actionName: "Import Style Library") { next in
                var merged = next.styleLibrary.normalized.styles
                for style in imported.styles {
                ...
                next.styleLibrary = GamepadStyleLibrary(styles: merged).normalized
            }
            selectedStyleTokenID = imported.styles.first?.id ?? selectedStyleTokenID
            resourceLibraryNotice = "Imported \(imported.styles.count) named \(imported.styles.count == 1 ? "style" : "styles")."
        }
    }

    private func handleAssetImport(_ result: Result<[URL], Error>) {
        handleResourceFileResult(result) { url, data in
            guard data.count <= GamepadAsset.maximumStoredBytes else {
                throw CocoaError(.fileReadTooLarge, userInfo: [NSLocalizedDescriptionKey: "Assets must be smaller than 2.5 MB."])
            }
            let contentType = UTType(filenameExtension: url.pathExtension)?.preferredMIMEType ?? "application/octet-stream"
            let asset = GamepadAsset(
            ...
            update(actionName: "Import Asset") { next in
                next.assetLibrary.assets.append(asset)
            }
            selectedAssetID = asset.id
            resourceLibraryNotice = "Imported \(asset.name)."
        }
    }

    private func handleResourceFileResult(
        _ result: Result<[URL], Error>,
        operation: (URL, Data) throws -> Void
    ) {
        switch result {
        case .success(let urls):
            ...
            do {
                try operation(url, Data(contentsOf: url))
            } catch {
                resourceLibraryNotice = error.localizedDescription
            }
        case .failure(let error):
    ...
    private var inspectorHeader: some View {
        VStack(alignment: .leading, spacing: Geist.Spacing.s2) {
            ...
            .overlay(Capsule().stroke(Geist.color(.grayAlpha400, scheme: colorScheme), lineWidth: accessibilityContrast == .increased ? 2 : 1))

            Picker("Inspector detail", selection: $inspectorMode) {
                ForEach(GamepadEditorInspectorMode.allCases) { mode in
                    Text(mode.displayName).tag(mode)
                }
            }
            .pickerStyle(.segmented)
            .labelsHidden()
            .help("Basic keeps the common action and size controls visible; Advanced adds styles, haptics, and effects")
        }
    }
    ...
    private var selectedElementInspector: some View {
        if case .controlBarItem(let item) = selectedControlID, selectedControlIsEditable {
            ...
            LazyVStack(alignment: .leading, spacing: 0) {
                ...
                Divider()
                inspectorAccordionSection(.selectedElementPosition, title: "Position & Size") {
                    VStack(alignment: .leading, spacing: Geist.Spacing.s4) {
    ...
    private var keypadLevelInspector: some View {
        LazyVStack(alignment: .leading, spacing: 0) {
            if let profileOutputModeContent {
                inspectorAccordionSection(.output, title: "Keypad Output") {
    ...
    private var selectedElementIdentitySection: some View {
        if let selectedLayerGroup,
           let groupItem = layerGroupItems.first(where: { $0.id == selectedLayerGroup.id }) {
            ...
            VStack(alignment: .leading, spacing: Geist.Spacing.s3) {
                ViewThatFits(in: .horizontal) {
                    HStack(spacing: Geist.Spacing.s2) { selectedElementQuickActions }
    ...
    private var alignmentButtons: some View {
        Menu {
            Button("Align Left Edges") { applySharedAlignment(.leftEdges) }
            Button("Align Horizontal Centers") { applySharedAlignment(.horizontalCenters) }
            Button("Align Right Edges") { applySharedAlignment(.rightEdges) }
            Divider()
            Button("Align Top Edges") { applySharedAlignment(.topEdges) }
            Button("Align Vertical Centers") { applySharedAlignment(.verticalCenters) }
            Button("Align Bottom Edges") { applySharedAlignment(.bottomEdges) }
        } label: {
        ...
        Menu {
            Button("Distribute Horizontal Centers") { applySharedDistribution(.horizontalCenters) }
            Button("Distribute Vertical Centers") { applySharedDistribution(.verticalCenters) }
            Divider()
            Button("Equal Horizontal Spacing") { applySharedDistribution(.horizontalSpacing) }
            Button("Equal Vertical Spacing") { applySharedDistribution(.verticalSpacing) }
        } label: {
            Label("Distribute", systemImage: "square.grid.3x3")
        }
        .geistButtonStyle(.secondary, size: .small)
    }
    ...
    private var selectedElementEffectsSection: some View {
        VStack(alignment: .leading, spacing: Geist.Spacing.s3) {
            ...
            selectedNormalSurfaceControls

            Divider()

            selectedLayeredShadowControls

            Divider()

            selectedPressedResponseControls
    ...
    private var selectedNormalSurfaceControls: some View {
        VStack(alignment: .leading, spacing: Geist.Spacing.s3) {
            Text("Surface")
                .geistTypography(.heading14)
                .foregroundStyle(Geist.color(.gray1000, scheme: colorScheme))
            ...
            LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible())], alignment: .leading, spacing: Geist.Spacing.s2) {
                colorTextField(title: "Stroke", value: visualStyleColorHexBinding(for: selectedControlID, keyPath: \.strokeColor), unit: nil)
                colorTextField(title: "Glow", value: visualStyleColorHexBinding(for: selectedControlID, keyPath: \.glowColor), unit: nil)
            }

            valueSlider(
                title: "Stroke width",
                value: visualStyleNumberBinding(for: selectedControlID, keyPath: \.strokeWidth),
                range: 0...12,
                valueText: String(format: "%.1f pt", Double(normalVisualStyleValue(for: selectedControlID).strokeWidth ?? 0))
            )
            valueSlider(
                title: "Glow",
                value: visualStyleNumberBinding(for: selectedControlID, keyPath: \.glowRadius),
                range: 0...64,
                valueText: "\(Int((normalVisualStyleValue(for: selectedControlID).glowRadius ?? 0).rounded())) pt"
            )
            valueSlider(
                title: "Opacity",
                value: visualStyleNumberBinding(for: selectedControlID, keyPath: \.opacity, defaultValue: 1),
                range: 0...1,
                valueText: "\(Int(((normalVisualStyleValue(for: selectedControlID).opacity ?? 1) * 100).rounded()))%"
            )
        }
    }

    private var selectedLayeredShadowControls: some View {
        let shadows = normalVisualStyleValue(for: selectedControlID).shadows ?? []
        return VStack(alignment: .leading, spacing: Geist.Spacing.s3) {
            HStack {
                Text("Layered Shadows")
                    .geistTypography(.heading14)
                    .foregroundStyle(Geist.color(.gray1000, scheme: colorScheme))
                Spacer()
                Button {
                    addVisualStyleShadow(for: selectedControlID)
                } label: {
                    Label("Add", systemImage: "plus")
                }
                .geistButtonStyle(.secondary, size: .small)
                .disabled(shadows.count >= 8)
            }

            if shadows.isEmpty {
                Text("Add up to eight outer shadows for raised, neon, or layered material effects.")
                    .geistTypography(.copy13)
                    .foregroundStyle(Geist.color(.gray900, scheme: colorScheme))
                    .fixedSize(horizontal: false, vertical: true)
            } else {
                ForEach(Array(shadows.indices), id: \.self) { index in
                    VStack(alignment: .leading, spacing: Geist.Spacing.s2) {
                        HStack {
                            Text("Shadow \(index + 1)")
                                .geistTypography(.label13)
                            Spacer()
                            Button {
                                removeVisualStyleShadow(at: index, for: selectedControlID)
                            } label: {
                                Image(systemName: "minus.circle")
                            }
                            .buttonStyle(.plain)
                            .accessibilityLabel("Remove shadow \(index + 1)")
                        }
                        colorTextField(title: "Color", value: visualStyleShadowColorBinding(for: selectedControlID, index: index), unit: nil)
                        LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible()), GridItem(.flexible())], spacing: Geist.Spacing.s2) {
                            inspectorMetricField(title: "Blur", value: visualStyleShadowNumberBinding(for: selectedControlID, index: index, keyPath: \.radius), unit: "pt", maxFractionDigits: 0)
                            inspectorMetricField(title: "X", value: visualStyleShadowNumberBinding(for: selectedControlID, index: index, keyPath: \.x), unit: "pt", maxFractionDigits: 0)
                            inspectorMetricField(title: "Y", value: visualStyleShadowNumberBinding(for: selectedControlID, index: index, keyPath: \.y), unit: "pt", maxFractionDigits: 0)
                        }
                    }
                    .padding(Geist.Spacing.s2)
    ...
    private var selectedPressedResponseControls: some View {
        let pressed = pressedVisualStyleValue(for: selectedControlID)
        return VStack(alignment: .leading, spacing: Geist.Spacing.s3) {
            Text("Pressed Response")
                .geistTypography(.heading14)
                .foregroundStyle(Geist.color(.gray1000, scheme: colorScheme))

            colorTextField(title: "Pressed fill", value: pressedFillHexBinding(for: selectedControlID), unit: nil)
            valueSlider(
                title: "Pressed scale",
                value: pressedVisualStyleNumberBinding(for: selectedControlID, keyPath: \.scale, defaultValue: 1),
                range: 0.5...1.5,
    ...
    private var filteredLayerListItems: [GamepadEditorLayerListItem] {
        let query = layerSearchText.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        guard !query.isEmpty else { return layerListItems }

        return layerListItems.compactMap { listItem in
            switch listItem {
            case .component(let item):
                return item.title.lowercased().contains(query) || item.subtitle.lowercased().contains(query) ? listItem : nil
            case .group(let group):
                ...
                return .group(
                    GamepadEditorLayerGroupItem(
                        ...
                        isLocationLocked: group.isLocationLocked
                    )
                )
            }
        }
    ...
    private func layerDragItemProvider(_ identities: [GamepadControlIdentity]) -> NSItemProvider {
        let selected = Set(identities)
        if isControlSelectionActive, !selectedControlIDs.isDisjoint(with: selected) {
            draggingLayerControlIDs = layerSelectionControlIdentities.filter { selectedControlIDs.contains($0) }
        } else {
            draggingLayerControlIDs = identities
        }

        let provider = NSItemProvider()
        let payload = draggingLayerControlIDs.map(\.id).joined(separator: "\n").data(using: .utf8) ?? Data()
        provider.registerDataRepresentation(forTypeIdentifier: Self.layerDragUTType.identifier, visibility: .all) { completion in
    ...
    private func moveDraggedLayers(_ identities: [GamepadControlIdentity], before targetIdentities: [GamepadControlIdentity]) {
        let movingSet = Set(identities)
        guard !movingSet.isEmpty,
              let target = targetIdentities.first(where: { !movingSet.contains($0) })
        else { return }

        let order = customization.orderedControlIdentitiesForDesign
        guard let targetIndex = order.firstIndex(of: target) else { return }
        update(actionName: identities.count > 1 ? "Reorder Layers" : "Reorder Layer") { next in
            next.moveLayers(movingSet, to: targetIndex)
        }
        selectedControlIDs = movingSet
        selectedControlID = identities.first ?? selectedControlID
        isControlSelectionActive = true
    }

    private func finishLayerDrag() {
        draggingLayerControlIDs.removeAll()
    }

    private func beginRenamingGroup(_ group: GamepadEditorLayerGroupItem) {
        groupRenameDraft = group.name
        groupRenameTargetID = group.id
    }

    private func commitGroupRename() {
        guard let groupRenameTargetID else { return }
        let name = groupRenameDraft.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !name.isEmpty else {
            self.groupRenameTargetID = nil
            return
        }
        var next = customization
        do {
            try next.renameLayerGroup(id: groupRenameTargetID, to: name)
            applyCustomization(next, undoActionName: "Rename Layer Group")
        } catch {
            editorOperationError = error.localizedDescription
        }
        self.groupRenameTargetID = nil
    }

    private func canRenameComponent(_ identity: GamepadControlIdentity) -> Bool {
        switch identity {
    ...
    private func beginRenamingComponent(_ identity: GamepadControlIdentity) {
    ...
    private var effectiveCanvasZoom: CGFloat {
        guard !isCanvasFitMode else { return 1 }
        return Self.clamp(
    ...
    private var canvasZoomPercentageText: String {
        isCanvasFitMode ? "Fit" : "\(Int((effectiveCanvasZoom * 100).rounded()))%"
    }

    private var canvasFocusCommandTitle: String {
        isCanvasFocusMode || (!isConfigurationSidebarVisible && !isInspectorSidebarVisible)
            ? "Exit Focus Canvas"
            : "Focus Canvas"
    }

    private func toggleCanvasFocusMode() {
        let animation: Animation? = accessibilityReduceMotion ? nil : .easeInOut(duration: 0.18)
        withAnimation(animation) {
            if isCanvasFocusMode {
                isConfigurationSidebarVisible = focusModeRestoresConfigurationSidebar
                isInspectorSidebarVisible = focusModeRestoresInspectorSidebar
                isCanvasFocusMode = false
            } else if !isConfigurationSidebarVisible && !isInspectorSidebarVisible {
                isConfigurationSidebarVisible = true
                isInspectorSidebarVisible = true
            } else {
                focusModeRestoresConfigurationSidebar = isConfigurationSidebarVisible
                focusModeRestoresInspectorSidebar = isInspectorSidebarVisible
                isConfigurationSidebarVisible = false
                isInspectorSidebarVisible = false
    ...
    private func adjustConfigurationSidebarWidth(grows: Bool) {
        let width = Self.clamp(
            configurationSidebarWidth + (grows ? 20 : -20),
            lower: Self.configurationSidebarMinWidth,
            upper: Self.configurationSidebarMaxWidth
        )
        configurationSidebarWidthValue = Double(width)
        draftConfigurationSidebarWidth = nil
        announceEditor("Setups sidebar is \(Int(width.rounded())) points wide.")
    }
    ...
    private func adjustInspectorSidebarWidth(grows: Bool) {
        let width = Self.clamp(
            inspectorSidebarWidth + (grows ? 20 : -20),
            lower: Self.inspectorSidebarMinWidth,
            upper: Self.inspectorSidebarMaxWidth
        )
        inspectorSidebarWidthValue = Double(width)
        draftInspectorSidebarWidth = nil
        announceEditor("Inspector sidebar is \(Int(width.rounded())) points wide.")
    }

    private func setCanvasZoom(_ zoom: CGFloat) {
        isCanvasFitMode = false
    ...
    private func setCanvasZoomAndCommit(_ zoom: CGFloat) {
        setCanvasZoom(zoom)
        commitCanvasZoomDraft()
    }
    ...
    private func fitCanvas() {
        ...
        canvasZoomValue = 1
        isCanvasFitMode = true
        announceEditor("Canvas fit to the available workspace.")
    }

    private func changeCanvasZoom(by delta: CGFloat) {
        setCanvasZoomAndCommit(effectiveCanvasZoom + delta)
    }

    private func zoomToSelection() {
        guard let selectionBounds = canvasSelectionBounds,
              canvasViewportSize.width > 1,
              canvasViewportSize.height > 1
        else { return }

        let fitWidth = max(120, canvasViewportSize.width - Self.canvasFitInset * 2)
        let fitHeight = max(120, canvasViewportSize.height - Self.canvasFitInset * 2)
        let fitScale = max(
            0.001,
            min(fitWidth / activeDeviceFrame.imageSize.width, fitHeight / activeDeviceFrame.imageSize.height)
        )
        let selectionInset: CGFloat = 96
        let desiredScale = min(
            max(80, canvasViewportSize.width - selectionInset) / max(selectionBounds.width, 44),
            max(80, canvasViewportSize.height - selectionInset) / max(selectionBounds.height, 44)
        )
        setCanvasZoomAndCommit(Self.clamp(desiredScale / fitScale, lower: 1, upper: Self.canvasZoomMax))
        canvasScrollRequestID = UUID()
        announceEditor("Zoomed to the selected control\(selectedControlIDs.count == 1 ? "" : "s").")
    }

    private var canvasSelectionBounds: CGRect? {
        guard isControlSelectionActive, !selectedControlIDs.isEmpty else { return nil }
        let frames = customization.resolvedControls(in: activeDesignCanvasSize)
            .filter { selectedControlIDs.contains($0.id) }
            .map(\.frame)
        guard let first = frames.first else { return nil }
        return frames.dropFirst().reduce(first) { $0.union($1) }
    }

    private func handleZoomShortcut(_ direction: Int) -> Bool {
        if direction == 0 {
            fitCanvas()
        } else {
            changeCanvasZoom(by: direction > 0 ? 0.10 : -0.10)
        }
        return true
    }
    ...
    private func setDeviceFrame(_ frame: GamepadEditorDeviceFrame) {
        if frame.orientation != selectedProfileOrientation,
           !hasSavedOrientation(frame.orientation) {
            pendingOrientationChange = GamepadPendingOrientationChange(frame: frame)
            return
        }
    ...
    private func completePendingOrientationChange(automaticallyArranged: Bool) {
        guard let pendingOrientationChange else { return }
        let destination = pendingOrientationChange.frame.orientation
        let source = selectedProfileOrientation
        self.pendingOrientationChange = nil
        transferOrientation(from: source, to: destination, automaticallyArranged: automaticallyArranged)
        setDeviceFrame(pendingOrientationChange.frame)
    }

    private func transferOrientation(
        from sourceOrientation: GamepadEditorDeviceOrientation,
        ...
        registerProfileUndoSnapshot(actionName: automaticallyArranged ? "Arrange Orientation" : "Copy Orientation")
        var profile = profiles[index]
        let currentCustomization = customization.normalized
        profile.setCustomization(currentCustomization, for: selectedProfileOrientation)
        profile.copyLayoutVariant(
            from: sourceOrientation == .landscape ? .landscape : .portrait,
            to: destinationOrientation == .landscape ? .landscape : .portrait,
            automaticallyArrange: automaticallyArranged
        )
        let transferred = profile.customization(for: destinationOrientation)
        if selectedProfileOrientation != destinationOrientation {
            profile.customization = currentCustomization
        }
        profiles[index] = profile.normalized

        if selectedProfileOrientation == destinationOrientation {
            applyCustomization(transferred)
        }
        persistProfiles()
    }
    ...
    private func clearLayoutQualityHighlight() {
    ...
    private func alignSelectedControls(_ axis: GamepadEditorAlignmentAxis) {
        applySharedAlignment(axis == .horizontalCenter ? .horizontalCenters : .verticalCenters)
    }

    private func distributeSelectedControls(axis: GamepadEditorDistributionAxis) {
        applySharedDistribution(axis == .horizontal ? .horizontalCenters : .verticalCenters)
    }

    private func applySharedAlignment(_ alignment: GamepadControlAlignment) {
        var next = customization
        do {
            if try next.alignControls(selectedControlIDs, alignment: alignment, in: currentCanvasLayoutSize) {
                applyCustomization(next, selectionSet: selectedControlIDs, undoActionName: "Align Controls")
            }
        } catch {
            editorOperationError = error.localizedDescription
        }
    ...
    private func applySharedDistribution(_ distribution: GamepadControlDistribution) {
        var next = customization
        do {
            if try next.distributeControls(selectedControlIDs, distribution: distribution, in: currentCanvasLayoutSize) {
                applyCustomization(next, selectionSet: selectedControlIDs, undoActionName: "Distribute Controls")
            }
        } catch {
            editorOperationError = error.localizedDescription
        }
    ...
    private func pressedVisualStyleValue(for identity: GamepadControlIdentity) -> GamepadControlStateStyle {
        selectedLayoutCustomization(for: identity).visualStyle?.pressed?.normalized ?? .empty
    }

    private func updatePressedVisualStyle(for identity: GamepadControlIdentity, mutate: (inout GamepadControlStateStyle) -> Void) {
        updateLayoutCustomization(for: identity) { layout in
            var visualStyle = layout.visualStyle ?? .empty
            var pressed = visualStyle.pressed ?? .empty
            mutate(&pressed)
            visualStyle.pressed = pressed.isEmpty ? nil : pressed.normalized
            layout.visualStyle = visualStyle.normalized
        }
    }

    private func pressedFillHexBinding(for identity: GamepadControlIdentity) -> Binding<String> {
        Binding(
            get: {
                guard case .solid(let color) = pressedVisualStyleValue(for: identity).fillStyle else { return "" }
                return color.hexString
            },
            set: { value in
                updatePressedVisualStyle(for: identity) { style in
                    let trimmed = value.trimmingCharacters(in: .whitespacesAndNewlines)
                    if trimmed.isEmpty {
                        style.fillStyle = nil
                    } else if let color = GamepadRGBAColor(hexString: trimmed) {
                        style.fillStyle = .solid(color.normalized)
                    }
                }
            }
        )
    ...
    private func pressedVisualStyleNumberBinding(
        for identity: GamepadControlIdentity,
        keyPath: WritableKeyPath<GamepadControlStateStyle, CGFloat?>,
        defaultValue: CGFloat
    ) -> Binding<Double> {
        Binding(
            get: { Double(pressedVisualStyleValue(for: identity)[keyPath: keyPath] ?? defaultValue) },
            set: { value in
                updatePressedVisualStyle(for: identity) { style in
                    let next = CGFloat(value)
                    style[keyPath: keyPath] = abs(next - defaultValue) < 0.001 ? nil : next
    ...
    private func addVisualStyleShadow(for identity: GamepadControlIdentity) {
        updateNormalVisualStyle(for: identity) { style in
            var shadows = style.shadows ?? []
            guard shadows.count < 8 else { return }
            shadows.append(
                GamepadControlShadowStyle(
                    color: GamepadRGBAColor(hexString: "#000000", alpha: 0.24) ?? .defaultValue,
                    radius: 12,
                    x: 0,
                    y: 6
                )
    ...
    private func removeVisualStyleShadow(at index: Int, for identity: GamepadControlIdentity) {
        updateNormalVisualStyle(for: identity) { style in
            var shadows = style.shadows ?? []
            guard shadows.indices.contains(index) else { return }
            shadows.remove(at: index)
            style.shadows = shadows.isEmpty ? nil : shadows
        }
    }

    private func visualStyleShadowColorBinding(for identity: GamepadControlIdentity, index: Int) -> Binding<String> {
        Binding(
            get: {
                let shadows = normalVisualStyleValue(for: identity).shadows ?? []
                guard shadows.indices.contains(index) else { return "" }
                return shadows[index].color.hexString
            },
            set: { value in
                guard let color = GamepadRGBAColor(hexString: value.trimmingCharacters(in: .whitespacesAndNewlines)) else { return }
                updateNormalVisualStyle(for: identity) { style in
                    var shadows = style.shadows ?? []
                    guard shadows.indices.contains(index) else { return }
                    shadows[index].color = color.normalized
                    style.shadows = shadows
                }
            }
        )
    ...
    private func visualStyleShadowNumberBinding(
        for identity: GamepadControlIdentity,
        index: Int,
        keyPath: WritableKeyPath<GamepadControlShadowStyle, CGFloat>
    ) -> Binding<Double> {
        Binding(
            get: {
                let shadows = normalVisualStyleValue(for: identity).shadows ?? []
                guard shadows.indices.contains(index) else { return 0 }
                return Double(shadows[index][keyPath: keyPath])
            },
            set: { value in
                updateNormalVisualStyle(for: identity) { style in
                    var shadows = style.shadows ?? []
                    guard shadows.indices.contains(index) else { return }
                    shadows[index][keyPath: keyPath] = CGFloat(value)
                    style.shadows = shadows
                }
            }
        )
    ...
    private func clearMaterialEffects(for identity: GamepadControlIdentity) {
        updateNormalVisualStyle(for: identity) { style in
            ...
            style.strokeWidth = nil
            style.glowColor = nil
            style.glowRadius = nil
            style.opacity = nil
            style.shadows = nil
    ...
    private func applyTemplateToCurrentSetup(_ template: GamepadControllerTemplate) {
        registerProfileUndoSnapshot(actionName: "Apply \(template.displayName) Template")
        var next = template.makeProfile().customization.normalized
        next.deviceCanvas = customization.deviceCanvas

        var mergedStyles = customization.styleLibrary.normalized.styles
        for style in next.styleLibrary.normalized.styles {
            if let index = mergedStyles.firstIndex(where: { $0.id == style.id }) {
                mergedStyles[index] = style
            } else {
                mergedStyles.append(style)
            }
        }
        next.styleLibrary = GamepadStyleLibrary(styles: mergedStyles).normalized

        var mergedAssets = customization.assetLibrary.normalized.assets
        for asset in next.assetLibrary.normalized.assets {
            if let index = mergedAssets.firstIndex(where: { $0.id == asset.id }) {
                mergedAssets[index] = asset
            } else {
                mergedAssets.append(asset)
            }
        }
        next.assetLibrary = GamepadAssetLibrary(assets: mergedAssets).normalized
        applyCustomization(next)
    }
    ...
    private var canDuplicateSelectedControls: Bool {
        interactionMode == .edit
            && isControlSelectionActive
            && !selectedControlIDs.isEmpty
            && selectedControlIDs.allSatisfy(isDuplicableControl)
    }

    private func isDuplicableControl(_ identity: GamepadControlIdentity) -> Bool {
        switch identity {
        case .builtin, .custom: true
        case .system, .controlBarItem: false
        }
    ...
    @discardableResult
    private func duplicateSelectedControls() -> Bool {
        guard canDuplicateSelectedControls
        else { return false }

        let selectedInLayerOrder = layerSelectionControlIdentities.filter { selectedControlIDs.contains($0) }
        var next = customization
        do {
            let duplication: GamepadElementDuplicationResult
            if let selectedLayerGroup {
                duplication = try next.duplicateLayerGroup(
                    id: selectedLayerGroup.id,
                    normalizedOffset: CGSize(
                        width: 12 / max(currentCanvasLayoutSize.width, 1),
                        height: 12 / max(currentCanvasLayoutSize.height, 1)
                    ),
                    canvasSize: currentCanvasLayoutSize
                ).elements
            } else {
                duplication = try next.duplicateControls(
                    selectedInLayerOrder,
                    normalizedOffset: CGSize(
                        width: 12 / max(currentCanvasLayoutSize.width, 1),
                        height: 12 / max(currentCanvasLayoutSize.height, 1)
                    ),
            ...
            let created = selectedInLayerOrder.compactMap { duplication.identityMap[$0] }
            guard !created.isEmpty else { return false }
            applyCustomization(
                next,
                selecting: created.first,
                selectionSet: Set(created),
                undoActionName: created.count > 1 ? "Duplicate Controls" : "Duplicate Control"
            )
            return true
        } catch {
            editorOperationError = error.localizedDescription
    ...
    private func deleteSelectedControl() -> Bool {
        guard interactionMode == .edit, selectedControlIsEditable else { return false }

        if selectedControlIDs.count > 1 {
            var next = customization
            var didChange = false
            for identity in selectedControlIDs {
                switch identity {
                case .custom(let id):
                    if next.customButtons.contains(where: { $0.id == id }) {
                ...
                case .controlBarItem(let item):
                    if next.controlBarItems.contains(item) {
            ...
            applyCustomization(next, selecting: preferredControlSelection(for: next), undoActionName: "Delete Controls")
            selectKeypadInspector()
            return true
        }
    ...
    private func performUngroupShortcut() -> Bool {
        guard let selectedLayerGroup else { return false }
        ungroupLayerGroup(selectedLayerGroup.id)
        return true
    }

    private func selectAllVisibleControls() -> Bool {
        guard interactionMode == .edit else { return false }
        let visibleIDs = Set(
            customization.resolvedControls(in: currentCanvasLayoutSize)
                .filter { !$0.layoutCustomization.isHidden }
                .map(\.id)
        )
        guard !visibleIDs.isEmpty else { return false }
        selectedControlIDs = visibleIDs
        selectedControlID = layerSelectionControlIdentities.first(where: { visibleIDs.contains($0) }) ?? visibleIDs.first ?? selectedControlID
        isControlSelectionActive = true
        expandedInspectorSections.insert(.selectedElementArrangement)
        return true
    }

    private func activateCanvasTool(_ tool: GamepadCanvasTool) -> Bool {
        guard interactionMode == .edit else { return false }
        activeCanvasTool = tool
        return true
    }

    private func handleEditorEscape() -> Bool {
    ...
    private func selectLayoutQualityIssue(_ issue: GamepadLayoutIssue) {
        let available = Set(componentListItems.map(\.identity))
        let identities = Set(issue.controls.compactMap(GamepadControlIdentity.init(stableID:)).filter { available.contains($0) })
        activeLayoutQualityIssue = issue
        announceEditor("Layout issue. \(issue.message)")
        DispatchQueue.main.async {
            isLayoutIssueAccessibilityFocused = true
        }
        ...
        guard !identities.isEmpty else {
            ...
            configurationSidebarTab = .layers
            isConfigurationSidebarVisible = true
            isInspectorSidebarVisible = true
            isCanvasFocusMode = false
            expandedInspectorSections.insert(.keypadComponents)
            return
        }

        highlightedQualityControlIDs = identities
        highlightedQualitySeverity = issue.severity
        selectedControlIDs = identities
        selectedControlID = layerSelectionControlIdentities.first(where: { identities.contains($0) }) ?? identities.first ?? selectedControlID
        isControlSelectionActive = true
        isInspectorSidebarVisible = true
        isCanvasFocusMode = false
        expandedInspectorSections.insert(identities.count > 1 ? .selectedElementArrangement : .selectedElementPosition)
        zoomToSelection()
    }

    private func applyLayoutRepair(_ repair: GamepadLayoutRepairKind, to issue: GamepadLayoutIssue?) {
        var next = customization
        let result = next.applyLayoutRepair(
            repair,
            issue: issue,
            canvasSize: activeDesignCanvasSize,
            respectingLocks: true
        )

        guard result.didChange else {
            let message = result.skippedLockedControlIDs.isEmpty
                ? "No controls needed \(repair.displayName.lowercased())."
                : "The affected controls are locked. Unlock them in Layers before applying this repair."
            announceEditor(message)
            editorOperationError = message
            return
        }

        let changedIdentities = Set(result.changedControlIDs.compactMap(GamepadControlIdentity.init(stableID:)))
        applyCustomization(
            next,
            selecting: changedIdentities.first ?? selectedControlID,
            selectionSet: changedIdentities,
            undoActionName: repair.displayName
        )
        highlightedQualityControlIDs = changedIdentities
        highlightedQualitySeverity = result.issueCountAfter < result.issueCountBefore ? .info : .warning
        activeLayoutQualityIssue = nil
        isControlSelectionActive = !changedIdentities.isEmpty
        expandedInspectorSections.insert(changedIdentities.count > 1 ? .selectedElementArrangement : .selectedElementPosition)
        zoomToSelection()

        var message = "\(repair.displayName) applied to \(changedIdentities.count) control\(changedIdentities.count == 1 ? "" : "s")."
        if !result.skippedLockedControlIDs.isEmpty {
            message += " \(result.skippedLockedControlIDs.count) locked control\(result.skippedLockedControlIDs.count == 1 ? " was" : "s were") skipped."
        }
        announceEditor(message)
    }

    private func primaryTestInput(for identity: GamepadControlIdentity) -> KeypadElementInputID? {
        guard let elementID = customization.elementID(for: identity) else { return nil }
        if case .custom(let id) = identity, customButton(id: id)?.normalized.isDecoration == true { return nil }
        let part: KeypadElementInputPart
        if case .custom(let id) = identity, customButton(id: id)?.normalized.isTrigger == true {
            part = .triggerDigital
        } else if case .custom(let id) = identity, customButton(id: id)?.normalized.isJoystick == true {
            part = .joystickUp
        } else {
            part = .primary
        }
        return KeypadElementInputID(elementID: elementID, part: part)
    }

    private func handleTestInputChanged(_ input: KeypadElementInputID, isPressed: Bool) {
        if isPressed {
            locallyPressedTestInputs.insert(input)
            lastTestActionText = customization.identity(forElementID: input.elementID).map(controlSelectionLabel(for:)) ?? "Test input"
        } else {
            locallyPressedTestInputs.remove(input)
        }
        onTestElementInputChanged?(input, isPressed)
    }

    private func pulseTestInput(_ input: KeypadElementInputID) {
        if let onTestElementInputTap {
            locallyPressedTestInputs.insert(input)
            lastTestActionText = customization.identity(forElementID: input.elementID).map(controlSelectionLabel(for:)) ?? "Test input"
            onTestElementInputTap(input, 120)
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.12) {
                locallyPressedTestInputs.remove(input)
            }
        } else {
            handleTestInputChanged(input, isPressed: true)
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.12) {
                handleTestInputChanged(input, isPressed: false)
            }
        }
    }

    private func releaseAllTestInputs() {
        let inputs = locallyPressedTestInputs
        locallyPressedTestInputs.removeAll()
        for input in inputs {
            onTestElementInputChanged?(input, false)
        }
    }
    ...
    private func exportProfiles(profileID: UUID?) {
        commitSelectedProfileNameDraft()
        commitPendingEditorChanges()
        onExportProfiles?(profiles, selectedProfileID, defaultProfileID, profileID)
    }
    ...
    private func moveProfile(_ profile: GamepadConfigurationProfile, by offset: Int) {
        guard let sourceIndex = profiles.firstIndex(where: { $0.id == profile.id }) else { return }
        let destinationIndex = min(max(sourceIndex + offset, 0), profiles.count - 1)
        guard destinationIndex != sourceIndex else { return }
        registerProfileUndoSnapshot(actionName: "Reorder Setups")
        let moved = profiles.remove(at: sourceIndex)
        profiles.insert(moved, at: destinationIndex)
        persistProfiles()
        announceEditor("Moved \(profile.name) \(offset < 0 ? "earlier" : "later") in the setup list.")
    }
    ...
    private func syncSelectedProfile(with newCustomization: GamepadCustomization, persistsImmediately: Bool = true) {
        ...
        let previousCustomization = nextProfile.customization(for: selectedProfileOrientation)
        nextProfile.setCustomization(normalizedCustomization, for: selectedProfileOrientation)

        if previousCustomization.styleLibrary.normalized != normalizedCustomization.styleLibrary.normalized
            || previousCustomization.assetLibrary.normalized != normalizedCustomization.assetLibrary.normalized {
            func syncResources(_ customization: inout GamepadCustomization) {
                customization.styleLibrary = normalizedCustomization.styleLibrary.normalized
                ...
                for index in customization.customButtons.indices {
                    if let styleID = customization.customButtons[index].layout.styleID,
                       !validStyleIDs.contains(styleID) {
                        customization.customButtons[index].layout.styleID = nil
                    }
                }
                if let styleID = customization.topBarActivationRegion.styleID,
                   !validStyleIDs.contains(styleID) {
                    customization.topBarActivationRegion.styleID = nil
                }
                for item in customization.controlBarItems {
                    var appearance = customization.controlBarItemCustomization(for: item)
                    if let styleID = appearance.styleID, !validStyleIDs.contains(styleID) {
                        appearance.styleID = nil
                        customization.setControlBarItemCustomization(appearance, for: item)
                    }
                }
            }
            syncResources(&nextProfile.customization)
            if var landscape = nextProfile.landscapeCustomization {
                syncResources(&landscape)
                nextProfile.landscapeCustomization = landscape
            }
            if var portrait = nextProfile.portraitCustomization {
                syncResources(&portrait)
                nextProfile.portraitCustomization = portrait
            }
        }
...
private struct GamepadEditorBlankSetupCard: View {
    ...
    var onApplyTemplate: (GamepadControllerTemplate) -> Void
    var onShowTour: () -> Void
    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: true) {
            ...
            LazyVGrid(columns: [GridItem(.adaptive(minimum: 116), spacing: Geist.Spacing.s2)], alignment: .leading, spacing: Geist.Spacing.s2) {
...
private struct GamepadLayerDropDelegate: DropDelegate {
    let targetIdentities: [GamepadControlIdentity]
    @Binding var draggingIdentities: [GamepadControlIdentity]
    let onMove: ([GamepadControlIdentity], [GamepadControlIdentity]) -> Void
    let onDropEnded: () -> Void

    func validateDrop(info: DropInfo) -> Bool {
        !draggingIdentities.isEmpty && Set(draggingIdentities).isDisjoint(with: targetIdentities)
    }
    ...
    func performDrop(info: DropInfo) -> Bool {
        guard !draggingIdentities.isEmpty else { return false }
        onMove(draggingIdentities, targetIdentities)
        onDropEnded()
        return true
    }
}
...
private struct GamepadEditorResizeHandle: View {
    ...
    let onAccessibilityAdjust: (Bool) -> Void
    ...
    var body: some View {
        ...
        .accessibilityHint(Text("Drag horizontally, or use increment and decrement, to resize"))
        .accessibilityAdjustableAction { direction in
            switch direction {
            case .increment: onAccessibilityAdjust(true)
            case .decrement: onAccessibilityAdjust(false)
...
private struct GamepadEditorKeyboardShortcutBridge: NSViewRepresentable {
    ...
    var onEscape: () -> Bool
    var onZoom: (Int) -> Bool
    var onTool: (GamepadCanvasTool) -> Bool
    ...
    func updateNSView(_ nsView: NSView, context: Context) {
        ...
        context.coordinator.onTool = onTool
    ...
    final class Coordinator: NSObject {
        ...
        init(
            ...
            onTool: @escaping (GamepadCanvasTool) -> Bool,
        ...
        private func handle(_ event: NSEvent) -> NSEvent? {
            ...
            if Self.isSelectAllEvent(event) {
                return onSelectAll() ? nil : event
            }

            if Self.isDuplicateEvent(event) {
                return onDuplicate() ? nil : event
            }

            if Self.isShortcutReferenceEvent(event) {
                return onShowShortcuts() ? nil : event
            }

            if let zoomDirection = Self.zoomDirection(for: event) {
                return onZoom(zoomDirection) ? nil : event
            }

            if let tool = Self.canvasTool(for: event) {
                return onTool(tool) ? nil : event
            }
            ...
            if Self.isUngroupEvent(event) {
                return onUngroup() ? nil : event
            }
        ...
        private static func isEscapeEvent(_ event: NSEvent) -> Bool {
            !event.isARepeat && event.keyCode == 53
        }

        private static func isSelectAllEvent(_ event: NSEvent) -> Bool {
            commandCharacterEvent(event, character: "a")
        }

        private static func isDuplicateEvent(_ event: NSEvent) -> Bool {
            commandCharacterEvent(event, character: "d")
        }

        private static func isShortcutReferenceEvent(_ event: NSEvent) -> Bool {
            commandCharacterEvent(event, character: "/")
        }

        private static func zoomDirection(for event: NSEvent) -> Int? {
        ...
        private static func canvasTool(for event: NSEvent) -> GamepadCanvasTool? {
            guard !event.isARepeat else { return nil }
            let flags = event.modifierFlags.intersection(.deviceIndependentFlagsMask)
            guard flags.intersection([.command, .option, .control, .shift]).isEmpty else { return nil }
        ...
        private static func commandCharacterEvent(_ event: NSEvent, character: String) -> Bool {
        ...
        private static func isUngroupEvent(_ event: NSEvent) -> Bool {
            guard !event.isARepeat else { return false }
            let flags = event.modifierFlags.intersection(.deviceIndependentFlagsMask)
            guard flags.contains(.command),
                  flags.contains(.shift),
...
private struct GamepadLayoutDesigner: View {
    ...
    var interactionMode: GamepadEditorInteractionMode = .edit
    ...
    @State private var activeMarquee: GamepadMarqueeSelectionState?
    @State private var activeTestInput: KeypadElementInputID?
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            ZStack(alignment: .topLeading) {
                ...
                .contentShape(Rectangle())
                .onTapGesture {
                    guard interactionMode == .edit, activeTool == .select else { return }
                    clearSelection()
                }
                .gesture(
                ...
                ForEach(controls) { control in
                    ...
                    let isPressed = control.elementID.map { elementID in
                        pressedElementInputs.contains { $0.elementID == elementID }
                    } ?? false
                    ...
                    GamepadDesignerButton(
                        ...
                        qualityHighlightSeverity: highlightedControlIDs.contains(control.id)
                            ...
                            : nil,
                        isTestMode: interactionMode == .test,
                        onAccessibilityTest: {
                            pulseAccessibilityTest(for: control)
                        },
                        onAccessibilitySelect: {
                            selectOnly(control.id)
                        },
                        onAccessibilityNudge: { direction in
                            accessibilityNudge(control, direction: direction, canvasSize: resolvedLayoutSize)
                        },
                        onAccessibilityResize: { grows in
                            accessibilityResize(control, grows: grows)
                        },
                        onAccessibilityRotate: { clockwise in
                            accessibilityRotate(control, clockwise: clockwise)
                        },
                        onAccessibilityRadiusAdjust: { increases in
                            accessibilityAdjustRadius(control, increases: increases)
                        },
                ...
                if interactionMode == .edit,
                   ...
                   !selectedDesignerControl.isLocationLocked {
                    GamepadControlRotationOverlay(
                        ...
                        onAccessibilityAdjust: { clockwise in
                            accessibilityRotate(selectedDesignerControl, clockwise: clockwise)
                        }
                    )
                    .position(
                ...
                if isMultiSelection,
                   let selectionFrame = selectionBounds(for: selectedDesignerControls) {
                    GamepadGroupSelectionOverlay(
                        ...
                        onAccessibilityResize: { grows in
                            accessibilityResizeSelection(selectedDesignerControls, grows: grows, canvasSize: resolvedLayoutSize)
                        },
                        onResizeChanged: { corner, value in
                ...
                if let activeMarquee {
                    Rectangle()
                        .fill(Geist.color(.blue700, scheme: colorScheme).opacity(0.12))
                        .overlay(Rectangle().stroke(Geist.color(.blue700, scheme: colorScheme), style: StrokeStyle(lineWidth: 1, dash: [4, 3])))
                        .frame(width: activeMarquee.rect.width * resolvedDisplayScale, height: activeMarquee.rect.height * resolvedDisplayScale)
                        .position(x: activeMarquee.rect.midX * resolvedDisplayScale, y: activeMarquee.rect.midY * resolvedDisplayScale)
                        .allowsHitTesting(false)
                }
                ...
                if interactionMode == .test {
                    Label("Test mode · Controls send real outputs", systemImage: "play.fill")
                        .geistTypography(.label12)
                        .padding(.horizontal, Geist.Spacing.s3)
                        .padding(.vertical, Geist.Spacing.s2)
                        .background(Geist.color(.background100, scheme: colorScheme).opacity(0.94), in: Capsule())
                        .overlay(Capsule().stroke(Geist.color(.blue700, scheme: colorScheme), lineWidth: 1))
                        .padding(Geist.Spacing.s3)
                        .allowsHitTesting(false)
                } else if activeTool.isDrawingShape {
                    drawingToolHint
                }
            }
            .contentShape(Rectangle())
            ...
            .gamepadDrawingCursor(isActive: activeTool.isDrawingShape)
            .clipShape(RoundedRectangle(cornerRadius: Geist.Radius.md, style: .continuous))
            .overlay(
                RoundedRectangle(cornerRadius: Geist.Radius.md, style: .continuous)
                    .stroke(Geist.color(.grayAlpha400, scheme: colorScheme), lineWidth: 1)
    ...
    private func marqueeGesture(
        ...
        DragGesture(minimumDistance: 4, coordinateSpace: .named("gamepadLayoutDesigner"))
            .onChanged { value in
                guard interactionMode == .edit, activeTool == .select else { return }
                let start = logicalPoint(from: value.startLocation, canvasSize: canvasSize, displayScale: displayScale)
                let current = logicalPoint(from: value.location, canvasSize: canvasSize, displayScale: displayScale)
                let baseSelection = activeMarquee?.baseSelection ?? (isShiftKeyPressed && isControlSelectionActive ? selectedControlIDs : [])
                let marquee = GamepadMarqueeSelectionState(startPoint: start, currentPoint: current, baseSelection: baseSelection)
                activeMarquee = marquee

                let intersecting = Set(
                    controls
                        .filter { !$0.layoutCustomization.isHidden && marquee.rect.intersects($0.frame) }
                        .map(\.id)
                )
                let nextSelection = baseSelection.union(intersecting)
                selectedControlIDs = nextSelection
                isControlSelectionActive = !nextSelection.isEmpty
                if let preferred = controls.first(where: { nextSelection.contains($0.id) })?.id {
                    selectedControlID = preferred
                }
            }
            .onEnded { _ in
                activeMarquee = nil
    ...
    private func pulseAccessibilityTest(for control: GamepadResolvedControl) {
        guard interactionMode == .test,
              !control.isDecoration,
              let elementID = control.elementID
        else { return }
        let part: KeypadElementInputPart = control.isJoystick ? .joystickUp : (control.isTrigger ? .triggerDigital : .primary)
        let input = KeypadElementInputID(elementID: elementID, part: part)
        onTestInputChanged(input, true)
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.12) {
            onTestInputChanged(input, false)
        }
    }

    private func updateTestInteraction(
        for control: GamepadResolvedControl,
        at displayLocation: CGPoint,
        displayScale: CGFloat
    ) {
        guard interactionMode == .test,
              !control.isDecoration,
        ...
        if control.isJoystick {
            let logicalPoint = CGPoint(
                x: displayLocation.x / max(displayScale, 0.001),
                y: displayLocation.y / max(displayScale, 0.001)
            )
            let deltaX = logicalPoint.x - control.center.x
            let deltaY = logicalPoint.y - control.center.y
            if abs(deltaX) > abs(deltaY) {
                part = deltaX < 0 ? .joystickLeft : .joystickRight
            } else {
                part = deltaY < 0 ? .joystickUp : .joystickDown
            }
        ...
        let input = KeypadElementInputID(elementID: elementID, part: part)
        guard activeTestInput != input else { return }
        if let activeTestInput {
            onTestInputChanged(activeTestInput, false)
        }
        activeTestInput = input
        onTestInputChanged(input, true)
    }

    private func endTestInteraction() {
        guard let activeTestInput else { return }
        onTestInputChanged(activeTestInput, false)
        self.activeTestInput = nil
    }
    ...
    private var drawingToolHint: some View {
        HStack(spacing: Geist.Spacing.s2) {
            Image(systemName: activeTool.systemImage)
                .font(.system(size: 11, weight: .semibold))
            Text("Click to place or drag to draw \(activeTool.displayName) · Esc to cancel")
                .geistTypography(.label12)
        }
        .foregroundStyle(Geist.color(.gray1000, scheme: colorScheme))
        .padding(.horizontal, Geist.Spacing.s3)
    ...
    private func updatePosition(
        ...
        activeAlignmentGuide = snapResult.guide
        let adjustedPosition = CGPoint(
            x: snapResult.frame.midX / max(canvasSize.width, 1),
            y: snapResult.frame.midY / max(canvasSize.height, 1)
        )
        var next = customization
    ...
    private func adjustedGroupTranslation(
        ...
        activeAlignmentGuide = GamepadAlignmentSnapSolver.framesAreEquivalent(snappedBounds, snapResult.frame) ? snapResult.guide : nil
    ...
    private func updateGroupResize(
        ...
        let resizedBounds = resizedGroupFrame(
    ...
    private func resizedGroupFrame(
    ...
    private func accessibilityNudge(
        ...
        direction: GamepadEditorNudgeDirection,
        canvasSize: CGSize
    ) {
        guard !control.isLocationLocked else { return }
        onBeginUndoableChange("Move Key")
        let nextCenter = CGPoint(
            x: control.center.x + direction.translation(step: 4).width,
            y: control.center.y + direction.translation(step: 4).height
        )
        let normalizedPosition = GamepadLayoutResolver.normalizedPosition(
            for: nextCenter,
    ...
    private func accessibilityResize(_ control: GamepadResolvedControl, grows: Bool) {
        guard !control.isLocationLocked else { return }
        onBeginUndoableChange("Resize Key")
        let multiplier: CGFloat = grows ? 1.1 : 0.9
        updateLayoutCustomization(for: control.id) { layout in
            layout.widthScale *= multiplier
            layout.heightScale *= multiplier
        }
        onEndEditingGesture()
    }

    private func accessibilityResizeSelection(
        _ controls: [GamepadResolvedControl],
        grows: Bool,
        canvasSize: CGSize
    ) {
        let unlocked = controls.filter { !$0.isLocationLocked }
        guard !unlocked.isEmpty, let bounds = selectionBounds(for: unlocked) else { return }
        let multiplier: CGFloat = grows ? 1.1 : 0.9
        onBeginUndoableChange("Resize Selection")
        for control in unlocked {
            updateLayoutCustomization(for: control.id) { layout in
                layout.widthScale *= multiplier
                layout.heightScale *= multiplier
            }
            let center = CGPoint(
                x: bounds.midX + (control.center.x - bounds.midX) * multiplier,
                y: bounds.midY + (control.center.y - bounds.midY) * multiplier
            )
            var next = customization
            next.setPosition(
                GamepadLayoutResolver.normalizedPosition(
                    for: center,
                    visualSize: CGSize(width: control.size.width * multiplier, height: control.size.height * multiplier),
                    in: canvasSize
                ),
                for: control.id
            )
    ...
    private func accessibilityRotate(_ control: GamepadResolvedControl, clockwise: Bool) {
        guard !control.isLocationLocked else { return }
        onBeginUndoableChange("Rotate Key")
        updateLayoutCustomization(for: control.id) { layout in
            layout.rotationDegrees = GamepadButtonCustomization.normalizedRotationDegrees(
                layout.rotationDegrees + (clockwise ? 15 : -15)
            )
        }
        onEndEditingGesture()
    }

    private func accessibilityAdjustRadius(_ control: GamepadResolvedControl, increases: Bool) {
        guard !control.isLocationLocked, control.shape.usesEditableCornerRadii else { return }
        let defaultRadius = control.shape.defaultEditableCornerRadius(in: control.size)
        let currentRadius = layoutCustomization(for: control.id).resolvedCornerRadii(defaultRadius: defaultRadius).averageRadius
        let maximumRadius = max(GamepadButtonCustomization.minimumCornerRadius, min(control.size.width, control.size.height) / 2)
        let nextRadius = Self.clamp(
            currentRadius + (increases ? 2 : -2),
            lower: GamepadButtonCustomization.minimumCornerRadius,
            upper: maximumRadius
        )
        guard abs(nextRadius - currentRadius) > 0.001 else { return }
        onBeginUndoableChange("Round Corners")
        updateLayoutCustomization(for: control.id) { layout in
            layout.cornerRadius = nil
            layout.cornerRadii = abs(nextRadius - defaultRadius) < 0.001 ? nil : .uniform(nextRadius)
...
private struct GamepadGroupSelectionOverlay: View {
    ...
    let onAccessibilityResize: (Bool) -> Void
    ...
    var body: some View {
        ...
        .accessibilityValue(Text("\(Int(selectionSize.width.rounded())) by \(Int(selectionSize.height.rounded())) points"))
        .accessibilityAdjustableAction { direction in
            switch direction {
            case .increment: onAccessibilityResize(true)
            case .decrement: onAccessibilityResize(false)
            @unknown default: break
            }
        }
    }
    ...
    private func resizeHandle(_ corner: GamepadResizeHandleCorner) -> some View {
        RoundedRectangle(cornerRadius: 3, style: .continuous)
            ...
            .accessibilityValue(Text("\(Int(selectionSize.width.rounded())) by \(Int(selectionSize.height.rounded())) points"))
            .accessibilityHint(Text("Drag, or use increment and decrement, to resize all selected components"))
            .accessibilityAdjustableAction { direction in
                switch direction {
                case .increment: onAccessibilityResize(true)
                case .decrement: onAccessibilityResize(false)
...
private struct GamepadControlRotationOverlay: View {
    ...
    private static let handleHitSize: CGFloat = 36
    private static let handleVisualSize: CGFloat = 24
    private static let handleIconFrame: CGFloat = 16
    private static let handleGap: CGFloat = 8
    private static let overlayOutset = handleGap + handleHitSize
    ...
    private var overlaySize: CGSize {
        CGSize(
            width: visualSize.width + Self.overlayOutset * 2,
            height: visualSize.height + Self.overlayOutset * 2
        )
    }
    ...
    private var handlePosition: CGPoint {
        CGPoint(
            ...
            y: Self.overlayOutset - Self.handleGap - Self.handleVisualSize / 2
        )
    }

    private var controlTopCenter: CGPoint {
        CGPoint(x: overlaySize.width / 2, y: Self.overlayOutset)
    }

    var body: some View {
        ZStack(alignment: .topLeading) {
            Path { path in
                path.move(to: CGPoint(x: handlePosition.x, y: handlePosition.y + Self.handleVisualSize / 2))
                path.addLine(to: controlTopCenter)
            }
            .stroke(
                Geist.color(.blue700, scheme: colorScheme).opacity(0.55),
    ...
    private var rotationHandle: some View {
        Image(systemName: "arrow.clockwise")
            ...
            .accessibilityValue(Text("\(Int(control.rotationDegrees.rounded())) degrees"))
            .accessibilityHint(Text("Press and drag, or use increment and decrement, to rotate it"))
            .accessibilityAdjustableAction { direction in
                switch direction {
                case .increment: onAccessibilityAdjust(true)
                case .decrement: onAccessibilityAdjust(false)
...
private extension View {
    ...
    @ViewBuilder
    func gamepadDrawingCursor(isActive: Bool) -> some View {
#if os(macOS)
        if isActive {
            modifier(GamepadCursorModifier(cursor: .crosshair))
        } else {
            self
        }
#else
...
private struct GamepadDesignerAccessibilityActionsModifier: ViewModifier {
    let isTestMode: Bool
    let supportsCornerRadius: Bool
    let onTest: () -> Void
    let onSelect: () -> Void
    let onNudge: (GamepadEditorNudgeDirection) -> Void
    let onResize: (Bool) -> Void
    let onRotate: (Bool) -> Void
    let onRadiusAdjust: (Bool) -> Void

    @ViewBuilder
    func body(content: Content) -> some View {
        if isTestMode {
            content.accessibilityAction { onTest() }
        } else {
            let editableContent = content
                .accessibilityAction { onSelect() }
                .accessibilityAction(named: Text("Show in Inspector")) { onSelect() }
                .accessibilityAction(named: Text("Move Left")) { onNudge(.left) }
                .accessibilityAction(named: Text("Move Right")) { onNudge(.right) }
                .accessibilityAction(named: Text("Move Up")) { onNudge(.up) }
                .accessibilityAction(named: Text("Move Down")) { onNudge(.down) }
                .accessibilityAction(named: Text("Rotate Counterclockwise")) { onRotate(false) }
                .accessibilityAction(named: Text("Rotate Clockwise")) { onRotate(true) }
                .accessibilityAdjustableAction { direction in
                    switch direction {
                    case .increment: onResize(true)
                    case .decrement: onResize(false)
                    @unknown default: break
                    }
                }
            ...
            if supportsCornerRadius {
                editableContent
                    .accessibilityAction(named: Text("Increase Corner Radius")) { onRadiusAdjust(true) }
                    .accessibilityAction(named: Text("Decrease Corner Radius")) { onRadiusAdjust(false) }
            } else {
                editableContent
            }
        }
...
private struct GamepadDesignerButton: View {
    ...
    var onAccessibilitySelect: () -> Void = {}
    var onAccessibilityNudge: (GamepadEditorNudgeDirection) -> Void = { _ in }
    var onAccessibilityResize: (Bool) -> Void = { _ in }
    var onAccessibilityRotate: (Bool) -> Void = { _ in }
    var onAccessibilityRadiusAdjust: (Bool) -> Void = { _ in }
    ...
    var body: some View {
        ZStack {
            ...
            if let qualityHighlightSeverity {
                RoundedRectangle(cornerRadius: 8 * inverseDisplayScale, style: .continuous)
                    .stroke(
                        qualityHighlightColor(qualityHighlightSeverity),
                        style: StrokeStyle(
                            lineWidth: 3 * inverseDisplayScale,
                            dash: [6 * inverseDisplayScale, 4 * inverseDisplayScale]
                        )
                    )
                    .padding(-4 * inverseDisplayScale)
            }
        ...
        .modifier(
            GamepadDesignerAccessibilityActionsModifier(
                ...
                onResize: onAccessibilityResize,
                onRotate: onAccessibilityRotate,
                onRadiusAdjust: onAccessibilityRadiusAdjust
            )
        )
    }
    ...
    private var accessibilityValue: String {
        ...
        if control.isLocationLocked { values.append("Locked") }
        values.append("X \(Int(control.frame.minX.rounded())), Y \(Int(control.frame.minY.rounded()))")
        values.append("\(Int(control.size.width.rounded())) by \(Int(control.size.height.rounded())) points")
        values.append("\(Int(control.rotationDegrees.rounded())) degrees")
    ...
    private var resolvedPresentation: GamepadResolvedControlPresentation {
        customization.resolvedPresentation(for: control, state: isPressed ? .pressed : .normal, scheme: colorScheme)
    }
    ...
    private func qualityHighlightColor(_ severity: GamepadLayoutIssueSeverity) -> Color {
        switch severity {
        case .error:
            Geist.color(.red900, scheme: colorScheme)
        case .warning:
            Geist.color(.amber900, scheme: colorScheme)
        case .info:
            Geist.color(.blue900, scheme: colorScheme)
        }
    }
    ...
    private func resizeHandle(_ corner: GamepadResizeHandleCorner) -> some View {
        RoundedRectangle(cornerRadius: 3 * inverseDisplayScale, style: .continuous)
            ...
            .frame(width: 24 * inverseDisplayScale, height: 24 * inverseDisplayScale)
            .contentShape(Rectangle())
            .highPriorityGesture(
                DragGesture(minimumDistance: 0, coordinateSpace: .named("gamepadLayoutDesigner"))
                    .onChanged { value in onResizeChanged(corner, value) }
            ...
            .accessibilityLabel(Text(corner.accessibilityLabel))
            .accessibilityHint(Text("Drag to resize this component, or use increment and decrement."))
            .accessibilityValue(Text("\(Int(control.size.width.rounded())) by \(Int(control.size.height.rounded())) points"))
            .accessibilityAdjustableAction { direction in
                switch direction {
                case .increment: onAccessibilityResize(true)
                case .decrement: onAccessibilityResize(false)
    ...
    private var radiusHandle: some View {
        Circle()
            ...
            .accessibilityValue(Text("\(Int(resolvedCornerRadii.averageRadius.rounded())) points"))
            .accessibilityHint(Text("Drag diagonally, or use increment and decrement, to round all corners"))
            .accessibilityAdjustableAction { direction in
                switch direction {
                case .increment: onAccessibilityRadiusAdjust(true)
                case .decrement: onAccessibilityRadiusAdjust(false)
                @unknown default: break
                }
            }
    }
    ...
    private func background(isSelected: Bool) -> some View {
        ...
        let lineWidth: CGFloat = isSelected ? (accessibilityContrast == .increased ? 4.5 : 3) : presentation.strokeWidth
...
private struct GamepadStyleLibraryJSONDocument: FileDocument {
    static var readableContentTypes: [UTType] { [.json] }
    static var writableContentTypes: [UTType] { [.json] }

    var library: GamepadStyleLibrary

    init(library: GamepadStyleLibrary = .empty) {
        self.library = library.normalized
    }
    ...
    init(configuration: ReadConfiguration) throws {
        ...
        library = try JSONDecoder().decode(GamepadStyleLibrary.self, from: data).normalized
    }

    func fileWrapper(configuration: WriteConfiguration) throws -> FileWrapper {
        let encoder = JSONEncoder()
        encoder.outputFormatting = [.prettyPrinted, .sortedKeys, .withoutEscapingSlashes]
        return FileWrapper(regularFileWithContents: try encoder.encode(library.normalized))
    }
}
#endif
