...
public struct PocketPadNativeSkinPreviewItem: Sendable {
...
public enum PocketPadNativeSkinPreviewError: LocalizedError {
    case invalidScale
...
@MainActor
public enum PocketPadNativeSkinPreviewRenderer {
    public static func writePNG(
        item: PocketPadNativeSkinPreviewItem,
        outputURL: URL,
        scale: CGFloat = 2
    ) throws {
    ...
    public static func writeContactSheet(
        items: [PocketPadNativeSkinPreviewItem],
        skinName: String,
        outputURL: URL,
        columns: Int = 4,
        scale: CGFloat = 1
        ...
        guard scale.isFinite, (0.5...4).contains(scale) else {
            throw PocketPadNativeSkinPreviewError.invalidScale
        }
        let snapshots = try items.map { item in
            PocketPadRenderedPreview(title: item.title, image: try render(item: item, scale: 2))
        }
        ...
        let view = PocketPadContactSheetView(
            skinName: skinName,
            rows: rows,
            cellSize: cellSize,
            spacing: spacing
        ...
        guard let image = renderer.cgImage else {
            throw PocketPadNativeSkinPreviewError.renderingFailed("contact sheet")
        }
        try FileManager.default.createDirectory(
            at: outputURL.deletingLastPathComponent(),
            withIntermediateDirectories: true
    ...
    public static func render(
        ...
        let view = PocketPadNativeSkinCanvas(
        ...
        renderer.isOpaque = true
        guard let image = renderer.cgImage else {
            throw PocketPadNativeSkinPreviewError.renderingFailed(item.title)
        }
        return image
    }
    ...
    private static func pngData(_ image: CGImage) throws -> Data {
        ...
        guard let destination = CGImageDestinationCreateWithData(
            ...
            1,
            nil
        ) else { throw PocketPadNativeSkinPreviewError.pngEncodingFailed }
        ...
        guard CGImageDestinationFinalize(destination) else {
            throw PocketPadNativeSkinPreviewError.pngEncodingFailed
        }
        return data as Data
    }
}

private struct PocketPadNativeSkinCanvas: View {
...
private struct PocketPadRenderedPreview {
    var title: String
    var image: CGImage
}

private struct PocketPadContactSheetView: View {
    let skinName: String
    let rows: [[PocketPadRenderedPreview]]
    let cellSize: CGSize
    let spacing: CGFloat
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: spacing) {
            VStack(alignment: .leading, spacing: 3) {
                ...
                Text("POCKETPAD NATIVE RENDERER · VARIANT & STATE REVIEW")
