...
- `reviews/` — approved art direction, versioned native-renderer evidence, and the pending human gate.
...
```bash
thumbconsole skin compile . -o build/solar-sumi-1.0.0.pocketpad --clean --strict
thumbconsole skin validate build/solar-sumi-1.0.0.pocketpad --strict
thumbconsole skin quality . --artboard arcade-stick-v1 --strict
thumbconsole skin preview . -o reviews/contact-sheet-4.png \
...
Publication, catalog changes, and approval are outside this workspace pass. `reviews/human-approval.json` must remain pending until a human reviews that exact contact sheet and package hash.
